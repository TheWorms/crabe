...
test('clic-document : un nouvel onglet qui sert un PDF — voie onglet, refermé derrière soi', async () => {
  ...
  assert.equal(resultat.ok, true);
  // La voie dit désormais AUSSI comment l'onglet a rendu ses octets (lot 80) :
  // « nouvel onglet — réponse déjà reçue », « — flux relu dans l'onglet »… Ce
  // détail part au journal du connecteur, où il prouve la voie mesurée au
  // premier passage réel.
  assert.match(resultat.voie, /^nouvel onglet — /);
  assert.match(resultat.voie, /réponse déjà reçue/);
  assert.deepEqual(resultat.buffer, PDF_FACTICE);
