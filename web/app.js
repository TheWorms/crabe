...
function showView(name) {
  ...
  closeMobileMenu();
  // La navigation ferme l'aperçu de la cloche (lot 79) : un panneau ancré n'a
  // de sens que sur l'écran où on l'a ouvert.
  ...
  if (name === 'notifications') $('tb-notifs')?.classList.add('active');
...
/**
 * La structure de l'accueil pendant le chargement (lot 72).
 *
 * L'accueil arrivait VIDE le temps que /home réponde. Dès l'arrivée, la grille
 * montre des blocs gris aux emplacements des blocs par défaut — mêmes classes
 * `.widget` et `.w-span-*` que les vrais : le contenu prend leur place sans
 * saut de mise en page. Aucune requête ici, c'est de l'affichage pur ; et le
 * squelette ne s'affiche qu'à la PREMIÈRE arrivée — au retour sur l'accueil,
 * les blocs d'une visite précédente restent visibles pendant que les données
 * se rafraîchissent, les remplacer par du gris serait un pas en arrière.
 */
const SQUELETTE_ACCUEIL = [
  { icon: 'grid', span: 12, lignes: 2 },
  { icon: 'chart', span: 12, lignes: 3 },
  { icon: 'sync', span: 6, lignes: 3 },
  { icon: 'alert', span: 6, lignes: 3 },
  { icon: 'doc', span: 12, lignes: 3 },
];

function renderHomeSkeleton() {
  const blocs = SQUELETTE_ACCUEIL.map(
    (bloc) => `
      <div class="widget w-span-${bloc.span}" aria-hidden="true">
        <div class="widget-head">
          <div class="w-icon">${WIDGET_ICONS[bloc.icon] || ''}</div>
          <div class="w-title"><span class="skl-line skl-titre"></span></div>
        </div>
        <div class="widget-body">${'<div class="skl-line"></div>'.repeat(bloc.lignes)}</div>
      </div>`
  ).join('');
  // Un seul indicateur, en tête de grille : la structure grise seule pouvait
  // passer pour une page cassée — le texte et la rotation disent que la page
  // travaille. Le lecteur d'écran a droit au même fait que l'œil (role=status).
  $('home-widgets').innerHTML =
    `<div class="skl-chargement" role="status"><span class="spinner" aria-hidden="true"></span>Chargement en cours…</div>${blocs}`;
}

/** Charge puis rend l'accueil. Toute erreur reste lisible dans la page. */
async function renderHome() {
  if (!home.data) renderHomeSkeleton();
  try {
    await loadHome();
  } catch (err) {
...
/**
 * « Voir les documents » depuis la fiche d'un connecteur : la liste s'ouvre
 * FILTRÉE sur lui.
 *
 * C'était le comportement d'origine (openPapiersFor filtrait sur le connecteur
 * courant) ; le retrait de « Mes Papiers » (09/08/2026) puis le retour de
 * l'écran « Mes documents » ont rebranché ce bouton sur la liste globale —
 * régression rapportée le 08/09/2026. Le filtre est celui du menu déroulant de
 * l'écran : on en ressort sur place (« Tous les services »). La recherche et
 * la période d'une visite précédente sont effacées — elles cacheraient les
 * documents qu'on vient précisément voir.
 */
function openDocsForConnector() {
  docs.query.connector = state.currentConnectorId || '';
  docs.query.q = '';
  docs.query.period = '';
  closeQuickview();
  showView('documents');
...
// ---------------------------------------------------------------------------
// L'écran « Vos notifications » (lot 76) et l'aperçu sous la cloche (lot 79)
//
// Mesuré avant le lot 76 : quinze notifications écrites en base depuis le
// lot 66, TOUTES non lues — les e-mails partaient, mais rien DANS
// l'application ne les rattrapait, et la notification du navigateur est
// morte-née sur une adresse en http:// (permission « refusée » d'emblée,
// mesurée au lot 66). Cet écran est le rattrapage : la liste vient de la base
// locale — aucune sonde réseau — la plus récente en tête, et chaque ligne
// mène à l'écran concerné.
//
// Le lot 79 ajoute deux choses :
//   - un APERÇU ancré sous la cloche : un clic ouvre les dernières non lues
//     (titre, une ligne de détail, ancienneté) avec un lien vers l'écran
//     complet ; il se ferme au clic à côté, à Échap et à la navigation ;
//   - la SUPPRESSION : une croix par ligne (geste unitaire, sans
//     confirmation — irréversible mais de faible enjeu), et « Tout
//     supprimer » sur l'écran complet, qui demande confirmation.
//
// Une notification dont l'objet a disparu (service désinstallé) reste lisible
// — son nom a été écrit avec elle, c'est l'acquis du lot 75 sur le journal de
// stockage.
// ---------------------------------------------------------------------------

const notifs = { data: [] };

/** Au-delà, l'aperçu renverrait un ascenseur : l'écran complet est à un clic. */
const APERCU_NOTIFS_MAX = 5;

/** L'état du panneau sous la cloche — fermé tant que personne ne clique. */
const panneauNotifs = { ouvert: false };

/** La pastille de la cloche : le NOMBRE de non-lues, rien au-dessous de une. */
function majBadgeNotifications(nonLues) {
  const badge = $('tb-notifs-count');
  if (!badge) return;
  const n = Number(nonLues) || 0;
  badge.style.display = n > 0 ? '' : 'none';
  badge.textContent = n > 99 ? '99+' : String(n);
}

/** Relève le compte des non-lues au démarrage, sans bloquer l'accueil. */
async function releverBadgeNotifications() {
  try {
    const { nonLues } = await api('/users/me/notifications/liste');
    majBadgeNotifications(nonLues);
  } catch {
    // Une cloche muette vaut mieux qu'un démarrage cassé : la liste reste
    // atteignable par l'écran lui-même.
  }
}

/**
 * La ligne qui dit pourquoi AUCUNE alerte du navigateur n'accompagne cet
 * écran — seulement quand c'est vrai (adresse non sécurisée). Le geste qui
 * les débloquerait est du ressort de qui héberge crabe, pas de crabe : on le
 * dit pour que personne ne cherche un réglage qui n'existe pas.
 */
function noteContexteNotifications(contexteSur) {
  if (contexteSur) return '';
  return 'Les alertes du navigateur ne peuvent pas fonctionner sur cette adresse : elle est '
    + 'servie en « http:// », et les navigateurs les réservent aux adresses sécurisées '
    + '(« https:// »). Les activer est un réglage de la machine qui héberge crabe, pas de '
    + 'crabe lui-même. Cet écran et les e-mails disent la même chose, sans rien manquer.';
}

/** Ce qu'une notification propose comme destination d'un clic. */
function cibleNotification(item) {
  if (!item?.id || item.id === 'chantier') {
    return { libelle: 'Voir les chantiers de fichiers', action: 'chantier', id: '' };
  }
  return { libelle: 'Ouvrir la fiche du service', action: 'connecteur', id: String(item.id) };
}

/** Une notification, prête à afficher. Pure : prouvable hors navigateur. */
function notificationHtml(n) {
  const nonLue = !n.seenAt;
  const lignes = (n.items || [])
    .filter((i) => i && (i.nom || i.message))
    .map((i) => {
      const cible = cibleNotification(i);
      return `<div class="notif-ligne">${esc(i.nom || i.id || '')}${
        i.message ? ` — ${esc(i.message)}` : ''
      } <a href="#" onclick="ouvrirCibleNotification('${esc(cible.action)}', '${esc(cible.id)}'); return false;">${esc(cible.libelle)}</a></div>`;
    })
    .join('');
  return `<div class="notif-item ${nonLue ? 'non-lue' : ''}" id="notif-${n.id}">
    <div class="notif-tete">
      <span class="notif-titre">${esc(n.title)}</span>
      <span class="notif-date" title="${esc(fmt.exact(n.createdAt))}">${esc(fmt.dateTime(n.createdAt))}</span>
      <button class="notif-croix" aria-label="Supprimer cette notification"
              title="Supprimer cette notification"
              onclick="supprimerNotifications([${Number(n.id)}])">✕</button>
    </div>
    ${lignes ? `<div class="notif-lignes">${lignes}</div>` : ''}
    <div class="notif-actions">
      ${nonLue
        ? `<button class="btn-mini" onclick="marquerNotificationsLues([${Number(n.id)}])">Marquer comme lu</button>`
        : '<span class="text-faint">Lue</span>'}
    </div>
  </div>`;
}

/**
 * L'aperçu sous la cloche (lot 79) : les dernières non lues — titre, une
 * ligne de détail, l'ancienneté (« il y a 13 h ») — chacune avec sa croix, et
 * le lien vers l'écran complet en haut à droite. Pure : prouvable hors
 * navigateur, comme l'écran du lot 76.
 */
function apercuNotificationsHtml(liste, nonLues) {
  const dernieres = (liste || []).filter((n) => n && !n.seenAt).slice(0, APERCU_NOTIFS_MAX);
  const lignes = dernieres
    .map((n) => {
      const premier = (n.items || []).find((i) => i && (i.nom || i.message));
      const detail = premier
        ? `${premier.nom || premier.id || ''}${premier.message ? ` — ${premier.message}` : ''}`
        : '';
      return `<div class="notif-pan-item">
        <div class="notif-pan-texte">
          <div class="notif-pan-titre">${esc(n.title)}</div>
          ${detail ? `<div class="notif-pan-detail">${esc(detail)}</div>` : ''}
          <div class="notif-pan-age" title="${esc(fmt.exact(n.createdAt))}">${esc(fmt.relative(n.createdAt))}</div>
        </div>
        <button class="notif-croix" aria-label="Supprimer cette notification"
                title="Supprimer cette notification"
                onclick="supprimerNotifications([${Number(n.id)}])">✕</button>
    ...
    .join('');
  const surplus = Math.max(0, (Number(nonLues) || 0) - dernieres.length);
  return `<div class="notif-pan-tete">
      <span class="notif-pan-entete">Notifications</span>
      <a href="#" class="notif-pan-lien" onclick="ouvrirHistoriqueNotifications(); return false;">Tout l'historique</a>
    </div>
    ${lignes || '<div class="notif-pan-vide">Rien de non lu.</div>'}
    ${surplus ? `<div class="notif-pan-plus">… et ${surplus} autre(s) non lue(s) dans l'historique.</div>` : ''}`;
}

/** L'écran entier. `contexteSur` est injecté pour rester prouvable en test. */
function notificationsHtml(liste, nonLues, contexteSur) {
  const note = noteContexteNotifications(contexteSur);
  const outils = `<div class="notifs-outils">
      ...
      <span>
        ${nonLues > 0 ? '<button class="btn-mini" onclick="marquerNotificationsLues(null)">Tout marquer comme lu</button>' : ''}
        ${liste.length ? '<button class="btn-mini" onclick="supprimerToutesNotifications()">Tout supprimer</button>' : ''}
      </span>
    </div>`;
  const corps = liste.length
    ? liste.map((n) => notificationHtml(n)).join('')
    : `<div class="empty-state">Aucune notification pour l'instant. crabe écrit ici ce qui
         s'est passé pendant que vous n'étiez pas là : une récupération planifiée qui échoue,
         une série de récupérations qui se termine, un long chantier qui s'achève.</div>`;
  return `${note ? `<div class="sec-note" style="margin-bottom:14px;">${esc(note)}</div>` : ''}
    ${outils}${corps}`;
}

async function renderNotifications() {
  const boite = $('notifs-content');
  if (!boite) return;
  try {
    const { notifications: liste, nonLues } = await api('/users/me/notifications/liste');
    notifs.data = liste || [];
    majBadgeNotifications(nonLues);
    boite.innerHTML = notificationsHtml(notifs.data, nonLues, window.isSecureContext === true);
  } catch (err) {
    boite.innerHTML = `<div class="empty-state">${esc(err.message)}</div>`;
  }
}

/**
 * Marque lu — certaines (`ids`) ou tout (`null`) — puis redessine.
 *
 * C'est le SEUL geste de l'écran : pas de suppression ici, encore moins en
 * masse. L'oubli définitif reste celui de la suppression du compte.
 */
async function marquerNotificationsLues(ids) {
  try {
    await api('/users/me/notifications/seen', {
      method: 'POST',
      body: ids ? { ids } : {},
  ...
  if (panneauNotifs.ouvert) await remplirPanneauNotifications();
}

/**
 * Supprime des lignes — la croix (lot 79). Pas de confirmation : le geste est
 * unitaire et de faible enjeu, même s'il est irréversible. Le compteur de la
 * cloche se met à jour avec la réponse, sans second appel.
 */
async function supprimerNotifications(ids) {
  try {
    const { nonLues } = await api('/users/me/notifications/supprimer', {
      method: 'POST',
      body: { ids },
    });
    majBadgeNotifications(nonLues);
  ...
  if (panneauNotifs.ouvert) await remplirPanneauNotifications();
  if ($('view-notifications')?.classList.contains('active')) await renderNotifications();
}

/** Vide tout l'historique — irréversible ET global : confirmé d'abord. */
async function supprimerToutesNotifications() {
  if (!confirm('Supprimer toutes vos notifications ? Cette suppression est définitive.')) return;
  try {
    const { nonLues } = await api('/users/me/notifications/supprimer', {
      method: 'POST',
      body: { tout: true },
...
// ---------------------------------------------------------------------------
// Le panneau sous la cloche (lot 79) — conduite
// ---------------------------------------------------------------------------

/** Relit la liste (base locale, comme l'écran) et redessine l'aperçu. */
async function remplirPanneauNotifications() {
  const pan = $('notif-panneau');
  if (!pan) return;
  try {
    const { notifications: liste, nonLues } = await api('/users/me/notifications/liste');
    notifs.data = liste || [];
    majBadgeNotifications(nonLues);
    pan.innerHTML = apercuNotificationsHtml(notifs.data, nonLues);
  } catch (err) {
    pan.innerHTML = `<div class="notif-pan-vide">${esc(err.message)}</div>`;
  }
}

/** Le clic de la cloche : ouvre l'aperçu ancré sous elle, ou le referme. */
async function basculerPanneauNotifications() {
  if (panneauNotifs.ouvert) return void fermerPanneauNotifications();
  const pan = $('notif-panneau');
  const cloche = $('tb-notifs');
  if (!pan || !cloche) return;
  panneauNotifs.ouvert = true;
  cloche.setAttribute('aria-expanded', 'true');
  // Ancré SOUS la cloche : aligné sur son bord droit, jamais hors écran.
  const rect = cloche.getBoundingClientRect();
  pan.style.top = `${Math.round(rect.bottom + 10)}px`;
  pan.style.right = `${Math.round(Math.max(8, window.innerWidth - rect.right - 4))}px`;
  pan.innerHTML = '<div class="notif-pan-vide">Chargement…</div>';
  pan.style.display = '';
  // L'ouverture s'anime par la classe (voir style.css) ; sous
  // prefers-reduced-motion, la transition est coupée là-bas.
  requestAnimationFrame(() => pan.classList.add('ouvert'));
  await remplirPanneauNotifications();
}

function fermerPanneauNotifications() {
  const pan = $('notif-panneau');
  if (!pan || !panneauNotifs.ouvert) return;
  panneauNotifs.ouvert = false;
  pan.classList.remove('ouvert');
  pan.style.display = 'none';
  $('tb-notifs')?.setAttribute('aria-expanded', 'false');
}

/** Le lien « Tout l'historique » de l'aperçu : l'écran complet du lot 76. */
function ouvrirHistoriqueNotifications() {
  fermerPanneauNotifications();
  showView('notifications');
}

// Fermé au clic à côté : tout clic hors du panneau et hors de la cloche.
document.addEventListener('click', (event) => {
  if (!panneauNotifs.ouvert) return;
  const pan = $('notif-panneau');
  if (pan?.contains(event.target)) return;
  if ($('tb-notifs')?.contains(event.target)) return;
  fermerPanneauNotifications();
});

/**
 * Le clic d'une ligne : la fiche du service concerné, ou l'écran des chantiers
 * de fichiers. Un service désinstallé ne mène nulle part — on le DIT, et la
 * notification reste lisible telle quelle.
 */
async function ouvrirCibleNotification(action, id) {
  if (action === 'chantier') {
    showView('profil');
    const entree = document.querySelector('[data-ppage="fichiers"]');
    if (entree) showProfilPage('fichiers', entree);
    return;
  }
  if (!state.connectors.length) await loadConnectors();
  const connecteur = state.connectors.find((c) => c.id === id);
  if (!connecteur) {
    showToast('Ce service n\'est plus installé — la notification reste là pour mémoire.');
    return;
  }
  openModal(id);
}
...
/**
 * Ce que la mise en pause CONSERVE (lot 82).
 *
 * ─── Pourquoi dire ce qui reste, et pas ce qui part ─────────────────────────
 *
 * Parce que rien ne part. La crainte, devant un interrupteur, c'est de perdre
 * quelque chose : la connexion au site — parfois ouverte à la main, dans la
 * fenêtre du navigateur, ce qui ne se refait pas d'un clic —, les documents
 * déjà rangés, les mois pas encore récupérés. Aucun des trois ne bouge. La
 * phrase le dit dans cet ordre-là, et elle s'affiche À CÔTÉ du geste plutôt
 * que dans une fenêtre de confirmation : le geste se défait d'un clic, une
 * confirmation ne ferait qu'ajouter du bruit devant un bouton réversible.
 */
const PAUSE_CONSERVE =
  'Rien n\'est perdu : sa connexion au site reste enregistrée, les documents déjà '
  + 'rangés ne bougent pas, et il rattrapera ce qu\'il aura manqué quand vous le '
  + 'rallumerez.';

/**
 * L'interrupteur de la vue Cartes.
 *
 * Un vrai `<button>`, et pas un `<div role="switch">` : lui seul se donne au
 * clavier (tabulation, Entrée, Espace) sans qu'on ait à réécrire ce que le
 * navigateur sait déjà faire. Le nom lu par un lecteur d'écran ne change pas
 * avec l'état — c'est `aria-checked` qui porte l'état, comme il se doit.
 */
function pauseSwitch(c) {
  const actif = !c.disabledAt;
  return `
        <button type="button" class="toggle${actif ? ' on' : ''}" role="switch"
                aria-checked="${actif ? 'true' : 'false'}"
                aria-label="Activer ou mettre en pause ${esc(c.name)}"
                title="${actif ? 'Mettre ce service en pause' : 'Rallumer ce service'}"
                onclick="basculerConnecteur('${esc(c.id)}', ${!actif}, this)"><span class="knob"></span></button>`;
}

/**
 * Les deux boutons de la vue Liste : celui de l'état courant est grisé — il
 * n'y a rien à faire —, l'autre est le geste. Une bascule d'un seul bouton
 * aurait obligé à deviner ce qu'il fait ; deux le disent.
 */
function pauseBoutons(c) {
  const actif = !c.disabledAt;
  const bouton = (label, vise, courant) =>
    `<button class="icon-btn${courant ? ' etat-courant' : ''}"`
    + (courant
      ? ' disabled aria-current="true" title="C\'est l\'état actuel de ce service.">'
      : ` onclick="basculerConnecteur('${esc(c.id)}', ${vise}, this)">`)
    + `${label}</button>`;
  return bouton('Activé', true, actif) + bouton('Désactivé', false, !actif);
}

/** La phrase, là où le geste se fait — sur la carte comme sur la ligne. */
function pauseNote(c) {
  if (!c.disabledAt) return '';
  return `<div class="pause-note">En pause (${esc(fmt.relative(c.disabledAt))}). ${esc(PAUSE_CONSERVE)}</div>`;
}

/** Les mêmes gestes des deux côtés : un seul endroit à corriger. */
function installedActions(c) {
  // ⚠ Grisé, pas retiré : un bouton qui disparaît fait douter de ce qu'on
  // vient de faire. Il revient de lui-même au rallumage.
  const enPause = !!c.disabledAt;
  const lancer = enPause
    ? '<button class="icon-btn" disabled '
      + 'title="Ce service est en pause — rallumez-le pour pouvoir le lancer.">Lancer maintenant</button>'
  ...
  return `
    ...
    ${c.health?.canSync !== false ? lancer : ''}`;
}

function installedRow(c) {
  return `
      <div class="installed-row${c.disabledAt ? ' en-pause' : ''}">
        ...
        ${installedBadge(c)}
        <div class="ir-actions">${pauseBoutons(c)}${installedActions(c)}</div>
        ${pauseNote(c)}
      </div>`;
}

/**
 * Met un service de côté, ou le rallume (lot 82).
 *
 * Aucune confirmation : le geste est réversible d'un clic, dans les deux sens,
 * et une fenêtre de plus à fermer serait du bruit. Le retour, lui, est visible
 * — même patron que le Store, qui annonce « EDF désinstallé ».
 */
async function basculerConnecteur(id, actif, bouton) {
  const nom = state.connectors?.find?.((c) => c.id === id)?.name || 'Ce service';
  if (bouton) bouton.disabled = true;
  try {
    await api(`/connectors/${id}/activation`, { method: 'PUT', body: { actif } });
    await renderProfilConnList();
    showToast(
      actif
        ? `${nom} rallumé — il reprend son rythme habituel.`
        : `${nom} en pause — sa connexion et ses documents sont conservés.`
    );
  } catch (err) {
    // Le bouton reprend la main : sans ça, un refus laisserait l'écran figé
    // sur un interrupteur mort, et l'utilisateur sans second essai.
...
function installedCard(c) {
  return `
  <div class="app-card${c.disabledAt ? ' en-pause' : ''}">
...
async function boot() {
  ...
  // La pastille de la cloche (lot 76) : un relevé au démarrage, puis elle vit
  // au rythme de l'écran « Vos notifications » lui-même — pas de boucle, pour
  // la même raison que releverNotifications.
  releverBadgeNotifications();
