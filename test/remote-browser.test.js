...
// ---------------------------------------------------------------------------
// Garde du lot 77 — aucune navigation vers une adresse `null`/`undefined`
// ---------------------------------------------------------------------------

test('adresseInvisitable : reconnaît le vide, le null/undefined et le chemin /null', () => {
  // Ce qui doit être refusé.
  for (const mauvaise of [
    '', '   ', null, undefined, 'null', 'undefined', 'NULL',
    'https://www.bricomarche.com/null',
    'https://www.bricomarche.com/null/',
    'https://www.bricomarche.com/undefined',
    'https://exemple.fr/UNDEFINED',
    'pas une url',
  ]) {
    assert.equal(remoteBrowser.adresseInvisitable(mauvaise), true,
      `${JSON.stringify(mauvaise)} devrait être jugée invisitable`);
  }
  // Ce qui doit passer : de vraies adresses, y compris quand « null » n'est
  // qu'une sous-chaîne d'un segment légitime (jamais le segment entier).
  for (const bonne of [
    'https://www.bricomarche.com/login',
    'https://www.bricomarche.com/my-account',
    'https://exemple.fr/nullify',
    'https://exemple.fr/compte/null-part-ailleurs',
    'https://exemple.fr/?redirect=null',
  ]) {
    assert.equal(remoteBrowser.adresseInvisitable(bonne), false,
      `${bonne} devrait être visitable`);
  }
});

test('start refuse une adresse d\'ouverture réduite à /null, AVANT toute fenêtre', async () => {
  const contexte = makeManager();

  await assert.rejects(
    () => contexte.manager.start(ouverture({ url: 'https://www.bricomarche.com/null' })),
    (err) => {
      assert.equal(err.statusCode, 400);
      assert.equal(err.expose, true);
      assert.equal(err.code, 'adresse-invisitable');
      // La phrase nomme le service et dit que c'est un défaut de configuration.
      assert.match(err.message, /Free Mobile/);
      assert.match(err.message, /incomplète ou invalide/);
      return true;
    }
  );

  // Rien n'a été alloué : ni processus, ni affichage, ni navigateur.
  assert.equal(contexte.processus.length, 0, 'aucun processus ne doit avoir été lancé');
  assert.equal(contexte.lancements.length, 0, 'aucun navigateur ne doit avoir été ouvert');
  assert.deepEqual(contexte.manager.reservedDisplays, [], 'aucun affichage ne doit rester pris');
});
