...
CREATE TABLE IF NOT EXISTS connector_installs (
  ...
  -- Mise en pause par l'utilisateur (UTC) ; NULL = le connecteur travaille.
  -- Volontairement séparé de `status`, qui dit l'état de SANTÉ : on met en
  -- pause un connecteur en erreur sans effacer son erreur (migration 57).
  disabled_at      TEXT,
...
CREATE TABLE IF NOT EXISTS destination_logs (
  ...
  -- Le nom de la destination AU MOMENT de l'écriture (lot 75). Un journal doit
  -- rester lisible quand son sujet disparaît : le volet « Cloud non utilisé »
  -- supprime la ligne de configuration d'un cloud sans copie réussie, et le
  -- nom résolu à l'affichage devenait alors un identifiant technique
  -- (« cloud-1f2e3d4c », mesuré le 09/09/2026 sur deux lignes MEGA du 26/08).
  dest_name TEXT,
  user_id   INTEGER,
