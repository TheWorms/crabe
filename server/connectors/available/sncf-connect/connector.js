...
async function fetchInvoices(config, ctx = {}) {
  ...
  return authSncf.surLeProfil({ id: ID, nom: NOM, ctx, urlDepart: URL_VOYAGES }, async (page) => {
    ...
    // La même vérité, déclarée pour la CARTE (lot 70) : le 08/09/2026, elle
    // disait « Aucune nouvelle facture » et se lisait « les derniers
    // justificatifs manquent » — alors que les voyages restants ne proposent
    // leur justificatif que par un envoi d'e-mail, que crabe ne déclenche
    // jamais. Le journal le disait ; l'écran, pas encore.
    return {
      accountId: null,
      invoices,
      precision: voyagesEmailSeul
        ? `${voyagesEmailSeul} voyage(s) sur ${voyages} ne proposent leur justificatif que par `
          + 'envoi d\'e-mail — crabe ne déclenche jamais cet envoi à votre place. Pour ces '
          + 'voyages, demandez-le vous-même sur SNCF Connect : Mes voyages → le voyage '
          + '→ « Vos justificatifs ».'
        : null,
    };
  });
