...
function safeSegment(value, fallback = 'inconnu') {
  ...
  // La coupe vient APRÈS le nettoyage, et peut donc le défaire : un nom long
  // coupé en plein milieu peut retomber sur un point (« …fact. »), que le
  // nettoyage précédent avait justement retiré. On repasse derrière la coupe —
  // c'est le seul ordre qui tienne dans les deux sens (lot 81).
  if (clean.length > MAX_SEGMENT) {
    clean = clean.slice(0, MAX_SEGMENT).trim().replace(/[. ]+$/, '');
  }
