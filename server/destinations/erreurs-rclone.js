...
/**
 * Masque les identifiants glissés dans une URL — « https://compte:motdepasse@
 * hôte/… » devient « https://[masqué]@hôte/… » (lot 75).
 *
 * Une adresse WebDAV se colle parfois avec ses identifiants dedans, et rclone
 * recopie l'adresse telle quelle dans ses erreurs (« Propfind "https://…" »).
 * Sans ce masque, le mot de passe finirait dans les journaux et sur les
 * pastilles de « Mes documents » — en clair, à demeure. Le masque est ENTIER :
 * un masquage qui laisse deviner (préfixe conservé, longueur révélée) n'en est
 * pas un — c'est un masque trop court qui a fait fuiter un mot de passe
 * d'application le 09/09/2026.
 */
function masquerIdentifiantsUrl(texte) {
  return String(texte || '').replace(/(\bhttps?:\/\/)[^\s/@]+(?::[^\s/@]*)?@/gi, '$1[masqué]@');
}
...
/**
 * Le détail technique d'une erreur inconnue : L'ERREUR, jamais l'avertissement.
 *
 * ─── L'incident qui a fixé cette règle (mesuré le 09/09/2026, lot 74) ────────
 *
 * Trois dépôts Proton Drive ont échoué avec pour tout détail : « proton drive
 * root link ID 'crabe/…': Forced to upload files to set modification times on
 * this backend. proton drive root link ID 'crabe/ ». Cette première ligne est
 * un AVERTISSEMENT d'rclone (Proton ne sait pas changer la date d'un fichier
 * sans le renvoyer) — l'erreur réelle venait APRÈS, et la coupe aux 160
 * PREMIERS caractères l'a mangée : trois échecs, et personne ne pouvait dire
 * pourquoi.
 *
 * La règle est donc : la DERNIÈRE ligne marquée ERROR / CRITICAL / Failed de
 * la sortie — c'est là qu'rclone écrit la cause, et la dernière occurrence est
 * le bilan final, pas une étape. À défaut de ligne marquée, les 160 DERNIERS
 * caractères : dans une sortie d'outil en ligne de commande, la conclusion est
 * à la fin, jamais au début.
 */
function detailUtile(texte) {
  const lignes = String(texte || '').split('\n').map((l) => l.trim()).filter(Boolean);
  const marquees = lignes.filter((l) => /(ERROR|CRITICAL|Failed)/i.test(l));
  // Les identifiants d'URL sont masqués AVANT la coupe : coupée d'abord, une
  // adresse pourrait perdre son « @ » et garder le mot de passe (lot 75).
  if (marquees.length) {
    return masquerIdentifiantsUrl(depouiller(marquees[marquees.length - 1])).slice(0, 160);
  }
  const tout = masquerIdentifiantsUrl(depouiller(texte));
  return tout.length <= 160 ? tout : `…${tout.slice(-160)}`;
}
...
function traduire(brut) {
  ...
  // Inconnue : le français d'abord, le détail ensuite — l'erreur finale, pas
  // l'avertissement qui la précède (voir detailUtile).
  const detail = detailUtile(texte);
  return (
