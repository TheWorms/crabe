...
const MIGRATIONS = [
  ...
  {
    id: 51,
    name: 'Uber : une annonce devenue connecteur repart en attente, pas en vitrine',
    // Le piège Materiel.net (migration 31), pour Uber au lot 72 : l'annonce a
    // inscrit sa ligne de catalogue en « available », et syncCatalog n'écrase
    // jamais — sans cette migration, l'ébauche jamais exercée contre un compte
    // réel apparaîtrait disponible pour tout le monde.
    up: remettreEnAttente('uber'),
  },
  {
    id: 52,
    name: 'SEPIG : l\'annonce devient ébauche, repart en attente',
    // Même piège que la 31/51 : SEPIG était ANNONCÉ (dossier `planned/`), sa
    // ligne de catalogue porte « available ». Le lot 73 écrit l'ébauche dans
    // `available/` avec `initialStatus: pending` ; sans cette migration,
    // syncCatalog (« ON CONFLICT DO NOTHING ») laisserait SEPIG en vitrine pour
    // tout le monde alors qu'aucun compte ne l'a jamais exercé.
    up: remettreEnAttente('sepig'),
  },
  {
    id: 53,
    name: 'journal de stockage : le nom s\'écrit avec la ligne, il ne se résout plus après coup',
    // Mesuré le 09/09/2026 : deux lignes MEGA du 26/08 affichées sous
    // « cloud-1f2e3d4c » — la destination supprimée, sa coquille nettoyée par
    // le volet « Cloud non utilisé » (lot 60), et le nom résolu à l'affichage
    // n'avait plus rien à résoudre. La colonne `dest_name` porte désormais le
    // nom au moment de l'écriture ; cette migration remplit ce qu'elle peut
    // encore (les lignes dont la destination existe toujours, et les
    // identifiants hérités de l'ancien modèle). Ce qui est déjà perdu reste
    // NULL : l'écran dit « destination supprimée » plutôt qu'un identifiant.
    up: nommerJournalStockage,
  },
  {
    id: 54,
    name: 'Ameli : le parcours réécrit sur session repart en attente, pas en vitrine',
    // Le lot 78 réécrit Ameli de fond en comble (identifiants → session par la
    // fenêtre, deux services au choix) : le nouveau parcours n'a JAMAIS été
    // exercé contre une session réelle — aucune session Ameli n'existait au
    // moment du lot. Même piège que Materiel.net (31) et Uber (51) : la ligne
    // de catalogue porte « available » depuis le lot 10, et syncCatalog
    // (« ON CONFLICT DO NOTHING ») n'appliquerait jamais l'initialStatus
    // « pending » du manifeste réécrit.
    up: remettreEnAttente('ameli'),
  },
  {
    id: 55,
    name: 'CAF : le connecteur désactivé ne part plus échouer tous les mois',
    // Le lot 78 remet CAF en « prochainement » (dossier planned/) sur décision
    // du compte observé — le portail a changé, rien n'est récupérable (mesure du lot
    // 77). Or le planificateur arme une tâche pour TOUTE installation
    // configurée, planification enregistrée ou pas (les défauts s'appliquent :
    // mensuel, activé) ; et l'exécution d'un service annoncé est refusée par
    // le registre. Sans cette migration, chaque mois écrirait un échec
    // « service annoncé » au journal, pour toujours. La planification est
    // DÉSARMÉE (enabled = 0), rien n'est supprimé : l'installation, ses
    // identifiants chiffrés et les documents déjà en base restent.
    up: desarmerPlanification('caf'),
  },
  {
    id: 56,
    name: 'AutoDoc et Coyote : les annonces deviennent connecteurs, repartent en attente',
    // Même piège que Materiel.net (31), Uber (51) et Ameli (54) : annoncés
    // (dossier planned/) depuis le lot 10, leurs lignes de catalogue portent
    // « available ». Le lot 79 écrit les vrais connecteurs dans available/
    // avec initialStatus « pending » — jamais exercés contre une session
    // réelle (aucune session n'existait au moment du lot : ni installation,
    // ni profil de navigateur, mesuré le 09/09/2026) — et syncCatalog
    // (« ON CONFLICT DO NOTHING ») n'appliquerait jamais ce statut.
    up: (database) => {
      remettreEnAttente('autodoc')(database);
      remettreEnAttente('coyote')(database);
    },
  },
  {
    id: 57,
    name: 'la pause d\'un connecteur : une date, distincte de son état de santé',
    // ─── Pourquoi une date, et pas un statut « désactivé » ──────────────────
    //
    // `status` répond à « est-ce que ça marche ? » (installed / needs-config /
    // error) ; `disabled_at` répond à « est-ce que ça doit travailler ? ».
    // Deux questions, deux colonnes. Les confondre coûterait exactement ce
    // qu'on veut éviter : un connecteur en erreur qu'on met en pause, puis
    // qu'on rallume, doit RETROUVER son erreur — pas repartir sur un état
    // neuf qui prétendrait que la panne s'est réglée toute seule.
    //
    // NULL = actif, et c'est le défaut : une base migrée ne change pas d'un
    // caractère, aucune installation existante ne se met en pause toute seule.
    up(database) {
      addColumn(database, 'connector_installs', 'disabled_at', 'TEXT');
    },
  },
];

/**
 * Migration 55 — désarme la planification d'un connecteur pour toutes ses
 * installations : une ligne de planification est ÉCRITE avec `enabled = 0`
 * (les défauts du planificateur s'appliquant à toute installation sans ligne,
 * supprimer la ligne ne désarmerait rien). Les autres réglages d'une ligne
 * existante sont conservés.
 */
function desarmerPlanification(connectorId) {
  return (database) => {
    if (!hasTable(database, 'connector_installs')) return;
    if (!hasTable(database, 'user_connector_schedules')) return;
    const installs = database
      .prepare('SELECT user_id FROM connector_installs WHERE connector_id = ?')
      .all(connectorId);
    const upsert = database.prepare(
      `INSERT INTO user_connector_schedules
         (user_id, connector_id, frequency, time_of_day, day_of_week, day_of_month,
          last_day_of_month, enabled, anchor_month)
       VALUES (?, ?, 'monthly', '03:00', 1, 1, 0, 0, NULL)
       ON CONFLICT(user_id, connector_id) DO UPDATE SET
         enabled    = 0,
         updated_at = datetime('now')`
    );
    for (const { user_id: userId } of installs) {
      upsert.run(userId, connectorId);
    }
  };
}
...
/**
 * Migration 53 — le journal de stockage porte le nom de son sujet (lot 75).
 *
 * Un journal qui résout le nom à l'affichage ne survit pas à la disparition de
 * son sujet : le nom s'écrit désormais AVEC la ligne (`logResult`), et cette
 * migration remplit `dest_name` partout où il peut encore l'être — depuis les
 * lignes de `destinations_config` (supprimées comprises : elles gardent leur
 * nom), et depuis les libellés des identifiants hérités de l'ancien modèle.
 * Une ligne dont le nom est déjà perdu reste NULL, et l'écran le dit en
 * français plutôt que d'afficher l'identifiant technique.
 *
 * Idempotente : seules les lignes encore NULL sont remplies.
 *
 * @param {import('better-sqlite3').Database} database
 */
function nommerJournalStockage(database) {
  if (!hasTable(database, 'destination_logs')) return;
  addColumn(database, 'destination_logs', 'dest_name', 'TEXT');

  const presets = require('../destinations/presets');
  const maj = database.prepare(
    'UPDATE destination_logs SET dest_name = ? WHERE dest_id = ? AND dest_name IS NULL'
  );

  if (hasTable(database, 'destinations_config')) {
    const lignes = database
      .prepare('SELECT dest_id, display_name, provider FROM destinations_config')
      .all();
    for (const ligne of lignes) {
      const nom = ligne.dest_id === 'local'
        ? 'Stockage local'
        : ligne.display_name || (presets.of(ligne.provider) || presets.of('autre')).label;
      maj.run(nom, ligne.dest_id);
    }
  }

  // Les identifiants d'avant le lot 25 (`proton`, `pcloud`…) : leur ligne a pu
  // disparaître, mais leur fournisseur reste nommable — même repli que le
  // catalogue pour les pastilles de facture.
  for (const [ancienId, providerId] of Object.entries(presets.HERITAGE)) {
    const herite = presets.of(providerId);
    if (herite) maj.run(herite.label, ancienId);
  }
}
