...
router.get('/storage', (req, res) => {
  ...
  // L'état ACTUEL de chaque destination, pour dire sur la ligne ce qui est
  // encore actif et ce qui est révolu (lot 75) : un échec d'il y a deux
  // semaines vers un cloud supprimé se lisait comme une tentative en cours.
  const lignesConfig = new Map(
    db
      .get()
      .prepare('SELECT dest_id, enabled, deleted_at FROM destinations_config')
      .all()
      .map((r) => [r.dest_id, r])
  );
  const etatDe = (destId) => {
    const ligne = lignesConfig.get(destId);
    if (!ligne || ligne.deleted_at) return 'supprimee';
    return ligne.enabled ? 'active' : 'desactivee';
  };

  const logs = db
    .get()
    .prepare(
      `SELECT d.id, d.dest_id, d.dest_name, d.at, d.success, d.message, u.username
    ...
    .all(...params)
    // Le nom ÉCRIT avec la ligne d'abord (lot 75) : c'est lui qui survit à la
    // disparition de son sujet. À défaut (lignes d'avant la migration 53), le
    // catalogue — qui nomme aussi les clouds supprimés tant que leur ligne
    // existe. Quand plus rien ne sait le nom (coquille nettoyée par le volet
    // « Cloud non utilisé »), `destName` reste null : l'écran dit
    // « Destination supprimée » plutôt qu'un identifiant technique.
    .map((row) => {
      const connue = lignesConfig.has(row.dest_id)
        || destinationCatalogue.brand(row.dest_id).name !== row.dest_id;
      return {
        ...row,
        dest_name: undefined,
        destName: row.dest_name
          || (connue ? destinationCatalogue.brand(row.dest_id).name : null),
        destEtat: etatDe(row.dest_id),
      };
    });
