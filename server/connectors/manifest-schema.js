...
function validateRemoteLogin(manifest, push) {
  ...
  /**
   * `domaineEspaceClient` (lot 73) — le domaine de l'espace client quand il
   * DIFFÈRE de celui de la boutique. Mesuré sur Atma le 08/09/2026 : les
   * comptes clients Shopify vivent sur `shopify.com`, pas sur la boutique —
   * un connecté qui demande `/account` est renvoyé vers
   * `shopify.com/<boutique>/account/orders`. Sans ce champ, le lot 68 juge ce
   * renvoi « vers un autre site » et REFUSE (garde `memeSite`) : juste pour un
   * fournisseur d'identité tiers, faux pour un espace client qui vit
   * légitimement ailleurs. Le déclarer fait d'un renvoi vers ce domaine la
   * preuve attendue — les gardes du lot 68 restent (statut ≥ 400 jamais une
   * preuve, renvoi vers une page de connexion = refus, vérifiés par
   * `remote-browser`). N'a de sens qu'avec `verifyUrl`, dont il juge le renvoi.
   */
  if (remote.domaineEspaceClient !== undefined) {
    const domaine = typeof remote.domaineEspaceClient === 'string'
      ? remote.domaineEspaceClient.trim() : null;
    if (!domaine) {
      push('remoteLogin.domaineEspaceClient doit être un nom de domaine (le domaine de '
        + 'l\'espace client quand il diffère de celui de la boutique)');
    } else if (!/^[a-z0-9.-]+\.[a-z]{2,}$/i.test(domaine)) {
      push(`remoteLogin.domaineEspaceClient : « ${domaine} » n'est pas un nom de domaine`);
    } else if (!(typeof remote.verifyUrl === 'string' && remote.verifyUrl.trim())) {
      push('remoteLogin.domaineEspaceClient sans remoteLogin.verifyUrl : aucune adresse '
        + 'dont juger le renvoi vers l\'espace client');
    }
  }
...
function normalizeRemoteLogin(remote) {
  ...
  return {
    ...
    // (Shopify héberge les comptes clients sur shopify.com). Même liste
    // blanche, même piège que `persistent` : sans cette ligne, la clé
    // disparaîtrait du manifeste chargé et le renvoi vers l'espace client
    // retomberait sous la garde `memeSite` du lot 68, qui le refuse.
    domaineEspaceClient: typeof remote.domaineEspaceClient === 'string'
      ? remote.domaineEspaceClient.trim() : '',
...
function publicView(manifest) {
  ...
  return {
    ...
    // serveur qui s'en sert, l'écran n'en fait rien. `domaineEspaceClient`
    // (lot 73) pareil : il ne sert qu'à juger un renvoi pendant le contrôle.
    remoteLogin: rest.remoteLogin
      ? (({ keepDomains, attendreUrls, renvoiAnonyme, domaineEspaceClient, ...visible }) =>
          visible)(rest.remoteLogin)
      : rest.remoteLogin,
