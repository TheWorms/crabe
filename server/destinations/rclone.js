...
/**
 * Le brouillon Proton Drive qui bloque un dépôt (lot 75).
 *
 * Signature MESURÉE en production le 09/09/2026 (destination_logs, un même
 * fichier en échec de 09:00 à 12:17) et lue dans la documentation du binaire
 * v1.75.0 (`rclone help backend protondrive`) : un envoi interrompu laisse un
 * « draft » côté Proton, et chaque envoi suivant du même fichier au même
 * endroit répond « a draft exist - usually this means a file is being
 * uploaded at another client, or, there was a failed upload attempt ».
 */
const SIGNATURE_BROUILLON_PROTON = /a draft exist/i;

/**
 * Copie un fichier local vers le remote.
 *
 * ─── Le brouillon Proton se remplace au SECOND essai, jamais au premier ──────
 *
 * L'option `--protondrive-replace-existing-draft=true` fait remplacer le
 * brouillon puis repartir l'envoi. Elle n'est PAS posée d'office : la
 * documentation du binaire prévient que si un autre client envoie vraiment au
 * même endroit au même moment, « the behavior is currently unknown » —
 * écraser en aveugle à chaque dépôt, c'est accepter ce comportement inconnu à
 * chaque fois. Le premier essai reste donc nu ; si — et seulement si — il
 * échoue sur la signature du brouillon, l'envoi se rejoue une fois avec le
 * remplacement : à ce moment-là, le brouillon qui bloque est un fait mesuré,
 * et ce que crabe redépose est le même document, relu d'le stockage local — au pire,
 * il remplace un envoi concurrent du même fichier par le même contenu.
 *
...
async function upload(dest, localFile, remoteRelPath) {
  return withConfig(dest, async (confFile) => {
    ...
    try {
      ...
      if (dest.type !== 'protondrive' || !SIGNATURE_BROUILLON_PROTON.test(texte)) throw err;
      await run(
        ['copyto', localFile, target, '--protondrive-replace-existing-draft=true'],
        { confFile }
      );
    }
    return target;
