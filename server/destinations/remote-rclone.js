 ...
 * @param {(normalise: {type: string, valeurs: object, rcloneConfig: string})
 *   => string|null} [definition.refusConfig] dit ce qui rend cette
 *   configuration INUTILISABLE (lot 75) — le message français qui explique le
 *   champ à remplir, ou `null`. Présent, il arme le refus avant service :
 *   aucun test, aucune copie, aucune mesure d'espace n'est tentée sur une
 *   configuration dont l'échec est certain par construction.
 */
function creerDestination({
  ...
  function normalizeConf(conf) {
    ...
    // ─── La configuration inutilisable, dite comme telle (lot 75) ────────────
    //
    // Distincte des autres refus avant service : ceux-là disent une session ou
    // un compte que le service n'acceptera pas ; celui-ci dit une
    // CONFIGURATION qui ne peut pas produire une connexion — l'adresse WebDAV
    // absente ou sans schéma. La distinction est exposée (`refusConfiguration`)
    // pour que la synchronisation et la carte puissent nommer le champ à
    // remplir sans confondre avec une reconnexion à refaire.
    const refusConfiguration = typeof refusConfig === 'function'
      ? refusConfig({ type, valeurs: valeurs || {}, rcloneConfig: bloc }) || undefined
      : undefined;
    ...
    return {
      ...
      // La configuration inutilisable passe en tête (lot 75) : aucun état de
      // session ne la rendra jouable, et son message nomme le champ à remplir.
      refusConfiguration,
      refusAvantService:
        refusConfiguration
  ...
  async function test(conf) {
    ...
    // Un refus décidable AVANT le service se dit tout de suite (lot 75) : une
    // configuration inutilisable — ou une session morte sans rien pour la
    // rouvrir — n'a pas besoin du binaire pour être refusée, et « Tester » la
    // carte kDrive sans numéro répondait « unsupported protocol scheme » en
    // anglais (mesuré les 07 et 09/09/2026). Même phrase que la copie et que
    // l'enregistrement : le champ à remplir.
    if (normalized.refusAvantService) {
      return { ok: false, message: `${name} : ${normalized.refusAvantService}` };
    }
