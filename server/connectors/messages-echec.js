...
/** Le jour et le mois d'un horodatage de la base (UTC), dans le fuseau donné. */
function jourMois(horodatage, zone) {
  const t = Date.parse(String(horodatage || '').replace(' ', 'T').replace(/(?<!Z)$/, 'Z'));
...
/**
 * Un refus d'accès APRÈS un succès s'appelle une expiration (lot 71).
 *
 * Le même refus brut ne veut pas dire la même chose selon ce qui a précédé :
 *
 *   - **jamais de réussite, session jamais confirmée** → le message du
 *     connecteur reste (c'est une première connexion qui échoue) — cette
 *     fonction rend `null` ;
 *   - **une récupération a déjà réussi**, ou **une session a été confirmée**
 *     (enregistrée avec preuve, lot 68), puis l'accès est refusé → « votre
 *     connexion a expiré », daté pour être vérifiable. L'utilisateur ne fait
 *     pas le même geste dans les deux cas : corriger une configuration qui
 *     n'a jamais marché, ou rouvrir une connexion qui marchait.
 *
 * Deux situations voisines ne passent JAMAIS par ici, et c'est l'appelant qui
 * les tient à l'écart :
 *
 *   - le site qui refuse le passage automatique malgré une session valide
 *     (lot 57, `err.renvoiAuthentification`) — on ne dit pas « expiré » à
 *     quelqu'un qui vient de se connecter ;
 *   - la preuve de liste introuvable (lot 31/68) — ce n'est pas un refus
 *     d'accès, son erreur ne porte pas `sessionExpired`.
 *
 * @param {{derniereReussite?: string|null, sessionOuverteLe?: string|null,
 *          zone?: string}} historique `derniereReussite` : la dernière
 *   récupération réussie (run_logs, UTC) ; `sessionOuverteLe` : l'ouverture de
 *   la session enregistrée (`__meta`, ISO) ; `zone` : le fuseau d'affichage.
 * @returns {string|null} le message d'expiration, ou `null` quand rien n'a
 *   jamais réussi — le message de première connexion du connecteur fait foi.
 */
function messageDeRefusExpire({ derniereReussite = null, sessionOuverteLe = null, zone = '' } = {}) {
  const reussite = jourMois(derniereReussite, zone);
  if (reussite) {
    return (
      `Votre connexion a expiré (dernière réussite le ${reussite}) — reconnectez-vous `
      + 'depuis la fiche du service.'
    );
  }
  const ouverture = jourMois(sessionOuverteLe, zone);
  if (ouverture) {
    return (
      `Votre connexion a expiré (connexion ouverte le ${ouverture}) — reconnectez-vous `
      + 'depuis la fiche du service.'
    );
  }
  return null;
