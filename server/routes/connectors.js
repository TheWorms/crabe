...
/**
 * Met le connecteur en pause, ou le rallume (lot 82).
 *
 * ─── Ce n'est PAS une désinstallation ───────────────────────────────────────
 *
 * Rien n'est effacé : ni les identifiants, ni la session ouverte à la main
 * dans la fenêtre du navigateur, ni la planification, ni les documents déjà
 * déposés. Seul `disabled_at` change. C'est réversible d'un clic, dans les
 * deux sens — raison pour laquelle le geste ne demande AUCUNE confirmation.
 *
 * ─── `actif` est exigé, explicitement ───────────────────────────────────────
 *
 * Un corps vide ne vaut ni « rallume » ni « mets en pause » : il est refusé.
 * Même règle qu'au lot 79 pour « tout supprimer » — une route qui écrit ne
 * devine jamais ce qu'on ne lui a pas dit.
 *
 * Le cloisonnement tient à `req.user.id` : la pause vaut pour CETTE
 * installation, donc pour ce compte-là. Deux comptes ayant installé le même
 * service se mettent en pause l'un sans l'autre.
 */
router.put(
  '/:id/activation',
  asyncHandler(async (req, res) => {
    ...
    if (actif !== true && actif !== false) {
      const err = new Error('Précisez si le service doit être actif ou en pause.');
      err.statusCode = 400;
      throw err;
    }

    if (!registry.getInstall(req.user.id, req.params.id)) {
      const err = new Error('Ce connecteur n\'est pas installé pour ce compte.');
      err.statusCode = 404;
      throw err;
    }

    const install = registry.setActivation(req.user.id, req.params.id, actif);
    // Le planificateur désarme (ou réarme) la tâche tout de suite : attendre un
    // redémarrage laisserait un service en pause partir une fois de plus.
    scheduler.reload();
    res.json({ ok: true, actif: !install.disabled_at, disabledAt: install.disabled_at || null });
  })
);
