...
/**
 * Les clés dont la VALEUR est un secret, dans une configuration ou un objet
 * de diagnostic (lot 75). Liste d'INCLUSION, volontairement large : mieux
 * vaut masquer un champ anodin qu'en laisser passer un.
 */
const MOTIF_CLE_A_MASQUER =
  /pass|pwd|secret|token|credential|api_?key|access_key|private|jeton|otp|2fa|totp|session|cookie|authorization|client_uid|_pin$|^pin$/i;

/**
 * Masque, RÉCURSIVEMENT et EN ENTIER, tout secret d'un objet.
 *
 * ─── L'incident qui a fixé les deux règles (lot 74 → 75) ─────────────────────
 *
 * Un mot de passe d'application a fui dans une sortie de diagnostic : le
 * masque était PLAT (il ne descendait pas dans `config.valeurs.pass`) et
 * PARTIEL (il laissait un préfixe « pour reconnaître la valeur »). D'où :
 *
 *   1. le parcours descend dans tous les objets et tableaux — un secret n'est
 *      jamais moins secret parce qu'il est rangé plus profond ;
 *   2. le remplacement est TOTAL (« [masqué] ») — un masquage qui laisse
 *      deviner un préfixe, une longueur ou un fragment n'en est pas un.
 *
 * Dans une chaîne libre, deux formes de secrets sont reconnues et masquées :
 * les lignes « clé = valeur » d'un bloc rclone collé, et les identifiants
 * d'URL (« https://compte:motdepasse@hôte »).
 *
 * @param {unknown} valeur l'objet (ou la valeur) à assainir — jamais modifié
 * @param {string} [cle] le nom sous lequel la valeur est rangée
 * @returns {unknown} une copie, secrets remplacés par « [masqué] »
 */
function masquerSecrets(valeur, cle = '') {
  if (Array.isArray(valeur)) return valeur.map((v) => masquerSecrets(v));
  if (valeur && typeof valeur === 'object') {
    return Object.fromEntries(
      Object.entries(valeur).map(([k, v]) => [k, masquerSecrets(v, k)])
    );
  }
  const secrete = MOTIF_CLE_A_MASQUER.test(String(cle || ''));
  if (typeof valeur !== 'string') {
    // Un nombre ou un booléen sous une clé secrète part aussi : un code à six
    // chiffres rangé comme nombre reste un code à six chiffres.
    return secrete && valeur !== null && valeur !== undefined ? '[masqué]' : valeur;
  }
  if (secrete) return valeur ? '[masqué]' : valeur;
  return valeur
    // Les lignes secrètes d'un bloc de configuration collé dans une chaîne.
    .replace(
      /^([ \t]*[A-Za-z0-9_-]*(?:pass|pwd|secret|token|otp|2fa|key)[A-Za-z0-9_-]*[ \t]*=[ \t]*)\S.*$/gim,
      '$1[masqué]'
    )
    // Les identifiants glissés dans une URL.
    .replace(/(\bhttps?:\/\/)[^\s/@]+(?::[^\s/@]*)?@/gi, '$1[masqué]@');
}
