 ...
 *
 * Forme MESURÉE le 09/09/2026 dans la documentation Infomaniak (FAQ 2409
 * « Se connecter à kDrive via WebDAV » et FAQ 2405, guide Windows/Qnap) :
 * `https://IDkDrive.connect.kdrive.infomaniak.com` — le numéro dans le nom
 * d'hôte, AUCUN chemin derrière. L'ancienne composition (lot 24) répétait le
 * numéro en chemin (`…/<numéro>`) : la documentation actuelle ne montre pas
 * ce segment, et un chemin en trop ferait viser un dossier qui n'existe pas.
 * Ces mêmes pages disent aussi : identifiant = adresse e-mail Infomaniak,
 * mot de passe = mot de passe d'APPLICATION — jamais celui du compte.
 */
function adresseWebdavKdrive(numero) {
  const propre = String(numero || '').trim().replace(/[^A-Za-z0-9-]/g, '');
  return propre ? `https://${propre}.connect.kdrive.infomaniak.com` : '';
}

/**
 * Les offres Infomaniak qui n'ouvrent PAS l'accès WebDAV — dit d'avance.
 *
 * MESURÉ le 09/09/2026 : une carte kDrive correctement remplie (numéro,
 * adresse, mot de passe d'application) recevait 403 dès la racine — vérifié
 * hors de crabe par un PROPFIND direct, donc rien à corriger dans crabe. La
 * cause est dans la documentation d'Infomaniak (FAQ 2408, « Importer des
 * données sur kDrive depuis Google Drive », lue le 09/09/2026) : l'accès
 * rclone/WebDAV y est marqué « ✘ INDISPONIBLE avec » kSuite Free / kSuite
 * Standard / my kSuite / my kSuite+ « (ik.me, etik.com, ikmail.com) ».
 *
 * Les noms d'offres et les domaines sont CEUX de cette page — rien d'inventé,
 * pas de catégorie « pro » ou « business » qui n'y figure pas. Si Infomaniak
 * change ses offres, c'est ici que ça se met à jour.
 *
 * Un AVERTISSEMENT, jamais un blocage : les offres évoluent, et une adresse
 * en ik.me peut être rattachée à un compte qui possède aussi une offre
 * ouverte. L'utilisateur enregistre s'il le veut — mais il sait d'avance.
 */
const DOMAINES_MY_KSUITE = ['ik.me', 'etik.com', 'ikmail.com'];

/** Le domaine my kSuite de l'adresse, ou `null` si elle n'en relève pas. */
function domaineMyKsuite(adresse) {
  const propre = String(adresse || '').trim().toLowerCase();
  return DOMAINES_MY_KSUITE.find((d) => propre.endsWith(`@${d}`)) || null;
}

/**
 * La phrase d'avertissement, UNE seule formulation pour la saisie comme pour
 * la carte enregistrée : deux textes du même fait finiraient par diverger.
 * `adresseDecrite` : « @ik.me » (le domaine reconnu), ou l'énumération
 * complète quand la phrase sert d'aide générique.
 */
function avertissementOffreKdrive(adresseDecrite) {
  return `Une adresse en ${adresseDecrite} correspond aux offres my kSuite et my kSuite+ `
    + 'd\'Infomaniak : ces offres n\'ouvrent pas l\'accès WebDAV, la connexion sera '
    + 'refusée. Vous pouvez tout de même enregistrer — les offres peuvent évoluer, et '
    + 'votre kDrive dépend peut-être d\'une autre offre que votre adresse — mais si la '
    + 'connexion échoue, c\'est la première cause à regarder.';
}
 ...
 */
/**
 * ─── L'aide tient sur UNE ligne par champ, le reste se replie (lot 75) ───────
 *
 * Mesuré sur la carte réelle : les aides de « Numéro » et « Mot de passe
 * d'application » laissaient chacune ~205 caractères TOUJOURS visibles (la
 * première ligne, avant le repli de `fieldHelp`) — un pavé, pour trois champs.
 * La règle est désormais : première ligne courte (l'essentiel + l'exemple),
 * tout le mode d'emploi derrière « Tout lire ». Rien n'est retiré — déplié,
 * le texte dit tout ce qu'il disait. Le numéro porte en plus un exemple
 * VISUEL (`exemple`, affiché en filigrane de la case) : la forme attendue,
 * jamais une valeur réelle (§1bis).
 */
const CHAMPS_KDRIVE = [
  {
    key: 'kdriveId',
    ...
    required: true,
    exemple: '123456',
    help: 'Un nombre : pour le kDrive n° 123456, l\'adresse de connexion sera '
      + 'https://123456.connect.kdrive.infomaniak.com.\n'
      + 'Vous lisez ce numéro dans l\'adresse de votre kDrive quand vous l\'ouvrez dans un '
      + 'navigateur : après « /kdrive/ », ou dans l\'adresse qui commence par '
  ...
  },
  {
    ...
    // ─── L'offre se reconnaît à l'adresse (lot 76) ───────────────────────────
    // Les domaines sont ceux que la documentation d'Infomaniak associe à
    // my kSuite / my kSuite+ (FAQ 2408, lue le 09/09/2026). L'avertissement
    // apparaît À LA SAISIE, avant l'enregistrement — et n'empêche rien.
    avertissementSaisie: {
      motif: '@(ik\\.me|etik\\.com|ikmail\\.com)$',
      message: avertissementOffreKdrive('@ik.me, @etik.com ou @ikmail.com'),
    },
  },
  {
    key: 'pass',
    ...
    required: true,
    obscurcir: true,
    // La première ligne garde le piège qui coûte le plus cher — ce n'est pas
    // le mot de passe du compte — et elle seule reste toujours visible.
    help: '⚠ Ce n\'est PAS le mot de passe de votre compte Infomaniak : créez un « mot de '
      + 'passe d\'application » dédié, et recopiez-le ici.\n'
      + 'Pourquoi : dès que la validation en deux étapes est active — et elle l\'est par '
      + 'défaut — Infomaniak refuse le mot de passe habituel pour ce genre de connexion.\n'
      + 'Pour le créer : sur manager.infomaniak.com, cliquez votre avatar en haut à droite, '
      + 'puis « Mot(s) de passe d\'application », puis « Ajouter ». Donnez-lui le nom que '
      + 'vous voulez (« crabe », par exemple) et recopiez ici la valeur affichée.\n'
...
function avertissements(type, normalise = {}) {
  ...
  // ─── L'offre kDrive sans WebDAV (lot 76) ──────────────────────────────────
  //
  // La configuration DÉJÀ enregistrée est examinée aussi, pour la même raison
  // que la clé TOTP plus haut : le contrôle à la saisie ne protège que
  // l'avenir, et la panne mesurée le 09/09/2026 était précisément une adresse
  // en ik.me posée avant lui — 403 à chaque dépôt, rien à l'écran pour le
  // dire. Le cas se reconnaît à la FORME : une destination `webdav` qui vise
  // `connect.kdrive.infomaniak.com` (ou qui porte un numéro de kDrive), avec
  // une adresse dans un domaine my kSuite. Un serveur WebDAV quelconque n'est
  // jamais inquiété, même si son identifiant est une adresse en ik.me.
  if (String(type) === 'webdav') {
    const adresseServeur = String(valeurs.url || '').trim()
      || (/^\s*url\s*=\s*(\S+)/m.exec(bloc)?.[1] || '');
    const estKdrive = /\.connect\.kdrive\.infomaniak\.com/i.test(adresseServeur)
      || Boolean(String(valeurs.kdriveId || '').trim());
    const domaine = domaineMyKsuite(
      String(valeurs.user || '').trim() || (/^\s*user\s*=\s*(\S+)/m.exec(bloc)?.[1] || '')
    );
    if (estKdrive && domaine) avis.push(avertissementOffreKdrive(`@${domaine}`));
  }
...
const PRESETS = {
  ...
  kdrive: {
    ...
    resume: 'Le stockage d\'Infomaniak, hébergé en Suisse. Les offres kSuite Free, kSuite '
      + 'Standard, my kSuite et my kSuite+ n\'ouvrent pas l\'accès dont crabe a besoin.',
    champs: CHAMPS_KDRIVE,
    // ─── La ligne TOUJOURS visible de la carte (lot 76) ──────────────────────
    // Ce qui exclut d'avance certaines offres se dit AVANT le 403, pas dans un
    // repli : le 09/09/2026, une carte bien remplie échouait sans qu'aucun
    // écran ne dise pourquoi. Le détail et le lien vers la page d'Infomaniak
    // vivent, eux, derrière « Tout lire » (même repli que les aides de champ).
    note: {
      ligne: 'Toutes les offres Infomaniak n\'ouvrent pas cet accès : avec kSuite Free, '
        + 'kSuite Standard, my kSuite ou my kSuite+ (les adresses en @ik.me, @etik.com et '
        + '@ikmail.com), la connexion sera refusée.',
      detail: 'crabe se connecte à kDrive par WebDAV, le chemin qu\'Infomaniak prévoit pour '
        + 'les outils comme lui. Sa documentation (« Importer des données sur kDrive depuis '
        + 'Google Drive », consultée le 09/09/2026) marque cet accès indisponible avec les '
        + 'offres kSuite Free, kSuite Standard, my kSuite et my kSuite+. Avec elles, '
        + 'Infomaniak refuse la connexion d\'emblée, quels que soient le numéro et le mot de '
        + 'passe saisis — ce n\'est pas un réglage de crabe. Les autres offres ouvrent cet '
        + 'accès normalement.',
      lien: {
        href: 'https://www.infomaniak.com/fr/support/faq/2408',
        texte: 'Lire la page d\'Infomaniak',
      },
    },
    // ─── `vendor = other`, décision re-mesurée au lot 76 ─────────────────────
    // La FAQ 2408 d'Infomaniak (lue le 09/09/2026) se contredit : son bloc à
    // coller dit `vendor = other`, son pas-à-pas dit « rclone (l'option
    // n°6) ». Mesuré sur le binaire du CT (v1.75, `rclone help backend
    // webdav`) : `rclone` y est documenté pour « rclone WebDAV server to
    // serve a remote over HTTP » — le cas où le SERVEUR est un rclone — et
    // active des particularités (en-tête X-OC-Mtime pour les dates de
    // modification) que kDrive n'annonce nulle part ; `other` est le WebDAV
    // standard, sans particularité. kDrive n'étant pas un serveur rclone,
    // `other` (choix du lot 74) est la valeur que la documentation du binaire
    // justifie : rien ne change.
    versChamps: (valeurs) => ({
