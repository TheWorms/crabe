...
test('deux domaines, un seul compte — et une seule connexion', () => {
  // ─── Ce que le lot 81 a remesuré ────────────────────────────────────────
  //
  // Ce test attendait une connexion sur atelierduportable.com (la vitrine) et
  // un champ de session. Les deux étaient périmés :
  //
  //   - le 11/08/2026, la connexion a été déplacée sur le domaine des
  //     COMMANDES parce que les cookies d'atelierduportable.com ne
  //     franchissaient pas la frontière de domaine — le connecteur restait
  //     bloqué là, sans une ligne au journal (commit « atelier-du-portable :
  //     connexion directe sur le domaine des commandes ») ;
  //   - ce connecteur n'a jamais eu de session par la fenêtre : il se connecte
  //     avec un identifiant et un mot de passe.
  //
  // Ce qui reste vrai, et que ce test garde : deux domaines pour un seul
  // compte, et un seul point de connexion à franchir.

  // La vitrine est l'autre domaine : c'est elle que le manifeste présente.
  assert.equal(MANIFESTE.site, 'www.atelierduportable.com');

  // Tout le parcours — connexion comprise — vit sur le domaine des commandes.
  assert.equal(atelier.URL_CONNEXION, 'https://www.piece-pc-portable.com/connexion/');
  ...
  // Une seule connexion à franchir : un identifiant, un mot de passe, et
  // aucune session à capturer dans une fenêtre.
  const types = MANIFESTE.fields.map((f) => f.type);
  assert.deepEqual(types.filter((t) => t === 'session'), []);
  assert.ok(types.includes('password'), 'la connexion se fait par mot de passe');
  assert.equal(MANIFESTE.remoteLogin, undefined);
});
