...
/**
 * Sursis accordé à un onglet ouvert VIDE par le site avant d'être refermé.
 *
 * Mesuré le 08/09/2026 sur Electro Dépôt : le parcours de connexion ouvre un
 * second onglet qui reste `about:blank` et qui devient l'onglet ACTIF — c'est
 * lui que noVNC montre, et l'utilisateur se retrouve devant un écran blanc
 * pendant que la page « Compte Client » vit dans l'onglet voisin (fenêtre
 * abandonnée au bout de 45 secondes ce jour-là). Un onglet vide n'est jamais
 * une page à montrer ni à juger — mais une fenêtre surgissante OAuth légitime
 * naît vide AUSSI, le temps que son adresse arrive : d'où ce sursis, large,
 * avant la fermeture. Refermer l'onglet vide rend la main à l'onglet du site.
 */
const DELAI_ONGLET_VIDE_MS = 5_000;
 ...
 */
/** L'adresse d'une page, ou '' quand elle est illisible (navigation en cours). */
function urlDe(page) {
  try {
...
/**
 * Un onglet « vide » : `about:blank` et les écrans internes du navigateur.
 * Jamais une page à juger — il ne dit rien du site ni de la session.
 */
function estOngletVide(url) {
  const adresse = String(url || '').trim();
  return !adresse || adresse.startsWith('about:') || adresse.startsWith('chrome:');
}

/**
 * Un chemin qui n'est qu'un `null`/`undefined` coercé en chaîne — la signature
 * d'une variable absente devenue segment d'adresse : `location = x` avec `x`
 * nul mène le navigateur vers `.../null`, `href = obj.champ` avec un champ
 * manquant vers `.../undefined`.
 */
const MOTIF_SEGMENT_NUL = /^\/(null|undefined)\/?$/i;

/**
 * Refuse une adresse impossible à visiter, avec une phrase qui dit quoi faire.
 *
 * Le 09/09/2026, le navigateur distant de Bricomarché s'est retrouvé sur
 * `www.bricomarche.com/null` — une 404 « La page demandée n'existe pas ». Une
 * adresse dont un morceau est un `null`/`undefined` coercé en chaîne ne mène
 * jamais nulle part de bon : mieux vaut lever ICI, avant la moindre
 * navigation, que d'ouvrir une fenêtre sur une page inexistante et laisser
 * l'utilisateur (et le diagnostic) deviner ce qui a manqué.
 *
 * @param {string} adresse
 * @returns {boolean} vrai si l'adresse est vide, littéralement `null`/
 *   `undefined`, illisible, ou d'un chemin `/null`|`/undefined`.
 */
function adresseInvisitable(adresse) {
  const brut = (adresse === null || adresse === undefined) ? '' : String(adresse).trim();
  if (!brut) return true;
  if (/^(null|undefined)$/i.test(brut)) return true;
  let parsee;
  try {
    parsee = new URL(brut);
  } catch {
    // Une adresse qu'`URL` ne sait pas lire n'est pas navigable : on la refuse
    // plutôt que de la passer telle quelle à `page.goto`, qui la résoudrait de
    // façon imprévisible (relative à la page courante).
    return true;
  }
  return MOTIF_SEGMENT_NUL.test(parsee.pathname);
}

/**
 * Rend l'adresse si elle est visitable, sinon lève une erreur exposable qui
 * nomme le service. La garde générale demandée après le `/null` de Bricomarché
 * (lot 77) : aucune navigation de la fenêtre ne part vers une adresse absente.
 *
 * @param {string} adresse
 * @param {string} service nom du connecteur, pour la phrase
 * @returns {string} l'adresse, inchangée
 */
function exigerAdresseVisitable(adresse, service = 'ce service') {
  if (adresseInvisitable(adresse)) {
    const montrable = (adresse === null || adresse === undefined || String(adresse).trim() === '')
      ? '(vide)'
      : String(adresse).trim();
    const err = new Error(
      `crabe n'ouvre pas la connexion à ${service} : l'adresse à visiter est incomplète `
        + `ou invalide (${montrable}). C'est un défaut de configuration de ce service, pas un `
        + 'problème de votre part — signalez-le à la personne qui administre crabe.'
    );
    err.statusCode = 400;
    err.expose = true;
    err.code = 'adresse-invisitable';
    throw err;
  }
...
/**
 * L'onglet à retenir parmi plusieurs — la leçon d'Electro Dépôt (08/09/2026).
 *
 * Le contrôle de session jugeait le DERNIER onglet ouvert ; le site venait
 * d'en ouvrir un resté `about:blank`, et c'est lui qui était jugé — un onglet
 * vide, à la place de la page « Compte Client » ouverte à côté. La règle,
 * générale parce que tout site qui ouvre un onglet au login est concerné :
 *
 *   - avec une adresse de référence : l'onglet qui PORTE le site (même
 *     domaine), le plus récent si plusieurs le portent ;
 *   - sinon : le plus récent des onglets non vides — un onglet vide n'est
 *     choisi que s'il n'existe rien d'autre ;
 *   - sans référence, on ne préfère PAS le site : la frappe doit atteindre
 *     la fenêtre surgissante d'un fournisseur d'identité (Google), qui vit
 *     sur un autre domaine.
 */
/**
 * Deux onglets portent-ils la MÊME page (origine + chemin) ?
 *
 * La comparaison ignore la requête et le fragment : c'est la page qui compte,
 * pas l'état qu'elle transporte. Sert à reconnaître un DOUBLON — le site qui
 * ouvre un second onglet de sa propre page de connexion (Boulanger,
 * 08/09/2026 : deux onglets « Me connecter », la saisie partie dans l'un,
 * l'utilisateur devant l'autre).
 */
function memeAdresseOnglet(a, b) {
  try {
    const x = new URL(String(a));
    const y = new URL(String(b));
    // Que des pages http(s) : deux `about:blank` partagent l'origine « null »
    // sans être la même page — les onglets vides ont leur règle à eux (lot 70).
    if (!/^https?:$/.test(x.protocol) || !/^https?:$/.test(y.protocol)) return false;
...
function choisirOnglet(pages, reference = '') {
  const vivantes = (pages || []).filter((p) => {
    try {
      return p && !p.isClosed?.();
    } catch {
      return false;
  ...
  if (reference) {
    const surLeSite = vivantes.filter((p) => preuve.memeSite(urlDe(p), reference));
    if (surLeSite.length) return surLeSite[surLeSite.length - 1];
  }
  const nonVides = vivantes.filter((p) => !estOngletVide(urlDe(p)));
  if (nonVides.length) return nonVides[nonVides.length - 1];
...
function createManager(overrides = {}) {
  ...
  // Réglable par les tests : prouver qu'un onglet resté vide est refermé ne
  // doit pas coûter cinq vraies secondes.
  const delaiOngletVideMs = overrides.delaiOngletVideMs || DELAI_ONGLET_VIDE_MS;
  ...
  async function start({
    ...
    // rendu déclaratif.
    renvoiAnonyme = '',
    // Lot 73 — le domaine de l'espace client quand il diffère de la boutique
    // (`remoteLogin.domaineEspaceClient`). Les comptes clients Shopify vivent
    // sur `shopify.com`, pas sur la boutique : un connecté qui demande
    // `/account` est renvoyé vers `shopify.com/<boutique>/account/orders`
    // (mesuré sur Atma le 08/09/2026). Le lot 68 jugerait ce renvoi « vers un
    // autre site » et refuserait (garde `memeSite`) ; déclaré, un renvoi vers
    // ce domaine devient la preuve — les gardes du lot 68 restent.
    domaineEspaceClient = '',
    ...
    // La garde du lot 77, avant TOUT le reste : une adresse d'ouverture absente
    // ou réduite à un `null`/`undefined` ne mérite ni fenêtre, ni Xvfb, ni
    // profil verrouillé — elle est refusée d'emblée, avec une phrase qui dit
    // que c'est le service qui est mal configuré (Bricomarché sur `/null`, le
    // 09/09/2026). `capture: false` (autorisation OAuth d'une destination)
    // n'est pas concerné : sa fenêtre part sur l'écran d'un fournisseur tiers,
    // pas sur une adresse déclarée par un connecteur — mais elle reste une URL
    // http(s) ordinaire, donc la garde la laisse passer sans rien changer.
    exigerAdresseVisitable(url, connectorName);
    if (current) {
      const err = new Error(
    ...
    const session = {
      ...
      // Lot 73 — le domaine de l'espace client déclaré, quand il diffère de la
      // boutique : un renvoi VERS lui est la preuve (Shopify, mesuré sur Atma).
      domaineEspaceClient: String(domaineEspaceClient || '').trim(),
   ...
   */
  /**
   * Referme les onglets ouverts par le site et restés vides (lot 70).
   *
   * Chromium montre l'onglet ACTIF — et un `about:blank` ouvert au clic de
   * connexion le devient (Electro Dépôt, 08/09/2026) : l'utilisateur voit un
   * écran blanc pendant que la page du site vit dans l'onglet voisin. Un
   * onglet resté vide au-delà du sursis (une fenêtre surgissante OAuth
   * légitime naît vide, le temps que son adresse arrive) est refermé — la
   * main revient à l'onglet du site. La page surveillée par le moteur n'est
   * jamais refermée, même vide : c'est elle que la détection lit.
   *
   * `vus` retient quand chaque onglet a été vu vide pour la première fois ;
   * un onglet qui a reçu une adresse entre-temps en sort aussitôt.
   */
  async function fermerOngletsVides(session, vus) {
    let pages = [];
    try {
      pages = session.browser?.context?.pages?.() || [];
    } catch {
      return; // contexte fermé : plus rien à trier
    }
    const encoreVides = new Set();
    for (const p of pages) {
      if (p === session.browser?.page) continue;
      let vivante = false;
      try {
        vivante = !p?.isClosed?.();
      } catch {
      ...
      if (!vivante || !estOngletVide(urlDe(p))) continue;
      encoreVides.add(p);
      const depuis = vus.get(p) ?? Date.now();
      vus.set(p, depuis);
      if (Date.now() - depuis < delaiOngletVideMs) continue;
      try {
        await p.close();
        encoreVides.delete(p);
        runtime.log(
          'info',
          `Session ${session.connectorName} : un onglet ouvert par le site et resté vide a été `
            + 'refermé — il masquait la fenêtre de connexion.',
          session.connectorId
        );
      } catch {
    ...
    for (const p of [...vus.keys()]) {
      if (!encoreVides.has(p)) vus.delete(p);
    }
  }

  /**
   * Referme les onglets en DOUBLE du site (lot 71).
   *
   * Boulanger, 08/09/2026 : le site ouvre un second onglet de sa propre page
   * de connexion. Deux onglets identiques, et AUCUN signal ne dit lequel est
   * au premier plan — mesuré sur le CT : sous les drapeaux d'automatisation,
   * `document.visibilityState` rend « visible » et `hasFocus()` rend vrai sur
   * les DEUX onglets d'une même fenêtre (Playwright désactive la mise en
   * sommeil des onglets d'arrière-plan). La saisie visait « le dernier onglet
   * non vide » ; l'utilisateur regardait l'autre.
   *
   * Puisque le premier plan ne se lit pas, on supprime l'ambiguïté : un
   * onglet qui duplique la page d'un onglet plus ancien (même origine, même
   * chemin) est refermé passé le même sursis que les onglets vides — le temps
   * qu'une navigation légitime lui donne une adresse à lui. La page
   * surveillée par le moteur n'est jamais refermée, et l'onglet survivant est
   * ramené au premier plan : ce que l'utilisateur voit redevient ce que crabe
   * lit et remplit.
   */
  async function fermerOngletsDoublons(session, vus) {
    let pages = [];
    try {
      pages = session.browser?.context?.pages?.() || [];
    } catch {
    ...
    const vivantes = pages.filter((p) => {
      try {
        return p && !p.isClosed?.() && !estOngletVide(urlDe(p));
      } catch {
        return false;
      }
    });
    const encoreDoubles = new Set();
    for (let i = 0; i < vivantes.length; i++) {
      const p = vivantes[i];
      if (p === session.browser?.page) continue;
      // Un doublon duplique un onglet PLUS ANCIEN (ou la page surveillée,
      // toujours née la première) : c'est le nouveau venu qui se referme,
      // jamais l'onglet où l'utilisateur travaille depuis le début.
      const jumeau = vivantes.find(
        (q, j) => q !== p && (j < i || q === session.browser?.page)
          && memeAdresseOnglet(urlDe(q), urlDe(p))
      );
      if (!jumeau) continue;
      encoreDoubles.add(p);
      const depuis = vus.get(p) ?? Date.now();
      ...
      try {
        await p.close();
        encoreDoubles.delete(p);
        await jumeau.bringToFront?.().catch?.(() => {});
        runtime.log(
          'info',
          `Session ${session.connectorName} : un onglet en double de la même page a été refermé — `
            + 'la fenêtre revient sur l\'onglet d\'origine, celui que crabe lit et remplit.',
          session.connectorId
        );
      } catch {
        /* déjà refermé : rien à faire */
    ...
    for (const p of [...vus.keys()]) {
      if (!encoreDoubles.has(p)) vus.delete(p);
    }
  }
  ...
  async function watch(session) {
    ...
    // Les onglets vides ouverts par le site, et depuis quand (lot 70).
    const ongletsVides = new Map();
    // Les onglets en double de la même page, et depuis quand (lot 71).
    const ongletsDoubles = new Map();
    ...
    if (!session.capture) {
      while (session === current && session.state === 'running') {
        await fermerOngletsVides(session, ongletsVides);
        await fermerOngletsDoublons(session, ongletsDoubles);
    ...
    while (session === current && session.state === 'running') {
      // Un onglet vide ouvert par le site cache la fenêtre : il se referme
      // même quand la boucle attend un geste de l'utilisateur (lot 70).
      await fermerOngletsVides(session, ongletsVides);
      await fermerOngletsDoublons(session, ongletsDoubles);
  ...
  async function sonderSession(session) {
    try {
      ...
      // L'écran à interroger est celui où l'utilisateur tape — le dernier
      // onglet non vide, jamais un `about:blank` ouvert par le site (lot 70).
      const saisie = await detection.attendUneSaisie(pageActive(session) || session.browser.page);
      if (saisie.attend) {
  ...
  async function save(session, { manuel = false } = {}) {
    ...
    try {
      ...
      // `naviguer: true` sur les DEUX chemins — clic « Enregistrer » et fin de
      // connexion détectée. Mesuré le 08/09/2026 sur Boulanger : le site dépose
      // l'utilisateur sur /account/my-orders/in-progress après le login, la
      // vérification automatique jugeait CETTE page (adresse de contrôle non
      // tenue, aucun marqueur — les achats en cours sont vides), déclarait la
      // session « non confirmée » et laissait la fenêtre sur les achats en
      // cours. Or arriver ici, c'est que la connexion est jugée finie
      // (detection.confirm, deux lectures) : conduire la fenêtre vers la page
      // de contrôle avant le verdict est la promesse du lot 49. La sonde sur
      // changement d'adresse (`sonderSession`), elle, ne navigue toujours pas.
      const essai = await essayerSession(session, state, { naviguer: true });
  ...
  async function essayerSession(session, state, { naviguer = false } = {}) {
    ...
    try {
      ...
      // La garde du lot 77 : une adresse de contrôle réduite à un `null` ne se
      // visite pas — on le dit comme un défaut de configuration, jamais comme
      // un verdict de session (elle atteint ici après le filtre `!verifyUrl`,
      // donc elle est non vide : reste à écarter un `/null` littéral).
      exigerAdresseVisitable(session.verifyUrl, session.connectorName);
      ...
      // ─── L'espace client vit sur un AUTRE domaine (lot 73) ───────────────
      //
      // Mesuré sur Atma le 08/09/2026 : un connecté qui demande
      // `atmakitchenware.fr/account` est renvoyé vers
      // `shopify.com/<boutique>/account/orders` — les comptes clients Shopify
      // sont hébergés sur shopify.com, pas sur la boutique. Le lot 68 jugerait
      // ce renvoi « vers un autre site » et refuserait (garde `memeSite`).
      // Quand le connecteur DÉCLARE ce domaine (`domaineEspaceClient`), un
      // renvoi VERS lui est la preuve — à condition que ce ne soit pas un
      // écran de connexion (l'anonyme, lui, finit sur
      // `shopify.com/authentication/…/login`, capté par estUrlAuthentification)
      // et que la page ne soit pas vide. Le statut ≥ 400 est déjà écarté par la
      // garde de cette branche : une session morte reçoit un 401/403, jamais
      // une page d'espace client.
      if (session.domaineEspaceClient && (statut === null || statut < 400)) {
        const urlFinale = String(page.url() || '');
        if (preuve.surDomaine(urlFinale, session.domaineEspaceClient)
          && !preuve.estUrlAuthentification(urlFinale)) {
          let corpsVide = false;
          try {
          ...
          if (!corpsVide) {
            return {
              verdict: 'confirmee',
              preuve: `${session.connectorName} : connexion confirmée — la page de contrôle a `
                + `renvoyé vers l'espace client hébergé sur ${session.domaineEspaceClient} `
                + `(URL finale ${urlFinale}), hors écran de connexion, or ce domaine ne sert `
                + `une page de compte qu'à une session établie `
                + `(${(state.cookies || []).length} cookie(s)).`,
            };
          }
        }
        // Hors du domaine déclaré, sur un écran de connexion, ou page vide : on
        // ne conclut pas ici — les gardes ci-dessous (tenue, renvoi, mur,
        // contrôle générique) tranchent, et refusent un renvoi vers la
        // connexion Shopify.
      }
   ...
   *
   * `naviguer` est vrai au moment d'ENREGISTRER — clic « Enregistrer » ou fin
   * de connexion détectée (lot 72 : le 08/09/2026, Boulanger déposait
   * l'utilisateur sur les achats en cours après le login, et la vérification
   * automatique jugeait cette page-là au lieu de conduire la fenêtre sur la
   * page de contrôle). Si la page courante ne porte pas la preuve, la fenêtre
   * est conduite sur la page de compte déclarée (`verifyUrl`) et relue — sauf
   * si un écran d'authentification est affiché : on n'emporte jamais un
   * formulaire sous les doigts de l'utilisateur. La sonde sur changement
   * d'adresse (`sonderSession`), elle, ne déplace JAMAIS la page.
   */
  async function essayerSurLaFenetre(session, { naviguer = false } = {}) {
    try {
      // La preuve se lit sur l'onglet du SITE, pas sur le dernier-né : un
      // `about:blank` ouvert par le parcours de connexion ne dit rien (lot 70).
      const page = pageDuSite(session);
      if (!page) {
        return {
          verdict: 'non-concluante',
          code: 'interrompu',
      ...
      if (naviguer && session.verifyUrl && !resultat.surFormulaire
  ...
  async function typeText(userId, connectorId, texte) {
    ...
    // ─── Deux onglets identiques (lot 71, Boulanger 08/09/2026) ─────────────
    //
    // Le premier plan ne se lit pas (voir `fermerOngletsDoublons`) : quand un
    // doublon de la page visée coexiste encore — le temps de son sursis —, la
    // saisie pourrait partir dans l'onglet que l'utilisateur ne regarde pas.
    // On le DIT au journal, et l'onglet visé est ramené au premier plan : ce
    // que l'utilisateur voit est l'endroit où le texte arrive.
    try {
      const doublon = (session.browser?.context?.pages?.() || []).some((p) => {
        try {
          return p && p !== page && !p.isClosed?.()
            && memeAdresseOnglet(urlDe(p), urlDe(page));
        } catch {
          return false;
        }
      });
      if (doublon) {
        runtime.log(
          'info',
          `Session ${session.connectorName} : deux onglets de la même page sont ouverts — `
            + 'la saisie vise le plus récent, ramené au premier plan.',
          session.connectorId
        );
        await page.bringToFront?.().catch?.(() => {});
      }
    } catch {
      /* contexte illisible : la saisie suit son cours normal */
    }
  ...
  function pageActive(session) {
    try {
      ...
      // Jamais un onglet vide quand il existe autre chose (lot 70) : le
      // dernier-né peut être un `about:blank` ouvert par le site, et frapper
      // dedans partirait dans le vide.
      const choisie = choisirOnglet(pages);
  ...
  /**
   * La page qui PORTE le site — celle que le contrôle de session doit juger.
   *
   * Même règle que `pageActive`, avec la préférence de domaine en plus : le
   * verdict se lit sur l'onglet du site (même domaine que l'adresse de
   * contrôle), le plus récent si plusieurs le portent — jamais sur un onglet
   * vide ouvert à côté (Electro Dépôt, 08/09/2026).
   */
  function pageDuSite(session) {
    try {
      const pages = session.browser?.context?.pages?.() || [];
      const choisie = choisirOnglet(pages, session.verifyUrl || session.url || '');
      if (choisie) return choisie;
    } catch {
      /* contexte fermé : on retombe sur la page d'origine */
    }
...
module.exports = {
  ...
  SESSION_TIMEOUT_MS,
  DELAI_FRAPPE_MS,
  DELAI_ONGLET_VIDE_MS,
  urlDe,
  estOngletVide,
  adresseInvisitable,
  exigerAdresseVisitable,
  MOTIF_SEGMENT_NUL,
  choisirOnglet,
  memeAdresseOnglet,
