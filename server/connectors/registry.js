  ...
  return listAll()
    ...
    .map((m) => {
      ...
      const connector = {
        ...
        // La mise en pause (lot 82). Une DATE, pas un booléen : l'écran peut
        // dire depuis quand, et `null` veut dire « actif » sans ambiguïté.
...
/**
 * Met un connecteur en pause, ou le rallume (lot 82).
 *
 * ─── Ce qui NE bouge pas ────────────────────────────────────────────────────
 *
 * Rien d'autre que `disabled_at`. Ni la configuration chiffrée, ni la session
 * enregistrée, ni la planification, ni les éléments découverts, ni `status`.
 * C'est toute la différence avec une désinstallation — qui, elle, efface la
 * session qu'un utilisateur a parfois ouverte à la main, à la fenêtre.
 *
 * `status` en particulier est laissé INTACT, y compris `error` : un connecteur
 * en panne qu'on met de côté le temps de s'en occuper doit retrouver sa panne
 * en revenant, pas un état neuf qui laisserait croire qu'elle s'est réglée.
 *
 * @param {number} userId
 * @param {string} connectorId
 * @param {boolean} actif `false` met en pause, `true` rallume
 * @returns {object|undefined} l'installation relue, ou undefined si absente
 */
function setActivation(userId, connectorId, actif) {
  db.get()
    .prepare(
      // Horodatage UTC, comme partout ailleurs dans la base.
      `UPDATE connector_installs
          SET disabled_at = ${actif ? 'NULL' : "datetime('now')"}
...
async function fetchInvoicesDetailed(connectorId, config, ctx = {}) {
  ...
  // ─── « Et il y a AUSSI ceci, que crabe ne fera pas » (lot 70) ─────────────
  //
  // Le quatrième cas, mesuré sur SNCF Connect le 08/09/2026 : la carte disait
  // « Aucune nouvelle facture » alors que 7 voyages sur 12 ne proposaient leur
  // justificatif que par envoi d'e-mail — un envoi que crabe ne déclenche
  // jamais. La vérité dormait dans le journal ; sur la carte, « rien de
  // neuf » se lisait « il en manque ». Le connecteur qui SAIT qu'une partie
  // des documents reste hors de sa portée le déclare (`precision`, une phrase
  // complète prête pour l'écran), et le planificateur l'ajoute au message —
  // quel que soit le compte de documents : la limite vaut aussi quand une
  // facture est descendue.
  const precision = String((Array.isArray(raw) ? '' : raw?.precision) || '').trim() || null;

  // ─── « Vu, tenté, et manqué » (lot 80) ────────────────────────────────────
  //
  // Le cinquième cas, mesuré sur Coyote le 10/09/2026 : 55 factures listées,
  // 55 tentées, 55 manquées — et l'écran disait « Succès — Aucune nouvelle
  // facture ». « Rien de nouveau » et « rien n'a marché » sont deux nouvelles
  // opposées ; les confondre est trompeur pour qui ne lira jamais le journal.
  // Le connecteur qui compte ce qu'il a TENTÉ sans l'obtenir le déclare, et le
  // planificateur en tire le statut de l'exécution. Un entier, borné : un
  // connecteur ne fabrique pas un échec avec une chaîne ou un nombre négatif.
  const brutManquees = Number(Array.isArray(raw) ? 0 : raw?.manquees);
  const manquees = Number.isFinite(brutManquees) && brutManquees > 0
    ? Math.floor(brutManquees)
    : 0;

  // Le parcours écourté (lot 80) dit pourquoi il s'est arrêté — une phrase
  // complète prête pour l'écran, sans grief technique : celui-ci reste au
  // journal. Sans manquée, un renoncement n'aurait aucun sens : il est ignoré.
  const renoncement = manquees > 0
    ? String((Array.isArray(raw) ? '' : raw?.renoncement) || '').trim() || null
    : null;
