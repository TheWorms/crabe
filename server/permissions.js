...
/**
 * L'appelant a-t-il le droit d'ATTRIBUER (ou de porter) ce rôle ?
 *
 * Un administrateur peut tout. Un gestionnaire délégué (rôle personnalisé
 * portant `users.manage` ou `roles.manage`) ne peut jamais accorder le rôle
 * administrateur, ni un rôle dont les permissions dépassent les siennes : on ne
 * délègue jamais plus de droits qu'on n'en détient soi-même. Sans cette garde,
 * `users.manage` suffisait à se promettre le rôle admin.
 */
function canGrantRole(actor, role) {
  if (!actor || !role) return false;
  if (actor.role === 'admin') return true;
  if (role.slug === ADMIN_SLUG) return false;
  const mine = new Set(permissionsForUser(actor));
  return permissionsForRole(role.id).every((permission) => mine.has(permission));
}

/**
 * L'appelant a-t-il le droit d'AGIR sur ce compte cible ?
 *
 * Un compte non administrateur ne peut jamais toucher à un administrateur
 * (changement de mot de passe, reset 2FA, révocation, suppression) : sinon
 * `users.manage` permettait de prendre le contrôle du vrai administrateur.
 */
