...
/**
 * « 12 juillet 2026 », « 12/07/2026 » ou « 5 oct. 2025 » → ISO.
 *
 * Les formes ABRÉGÉES (« oct. », « juil. » — celles de l'espace client Shopify,
 * mesurées sur Atma le 09/09/2026, lot 74) se reconnaissent au PRÉFIXE : chaque
 * abréviation française usuelle est le début du nom complet, la même table sert
 * donc aux deux formes. Le préfixe le plus long gagne (« juin » avant « juil. »
 * n'est pas un risque : aucun n'est préfixe de l'autre — mais « mai » l'est de
 * « mais », d'où l'exigence d'une correspondance de MOT entier avant le point).
 */
function normalizeFrenchDate(raw) {
  ...
  if (long) {
    const mot = long[2];
    const complet = Object.keys(months).find((nom) => nom === mot || nom.startsWith(mot));
    // Deux lettres ne désignent rien sans ambiguïté (« ja » ? « juin » ou
    // « juillet » pour « ju » ?) : en dessous de trois, on refuse de deviner.
    if (complet && mot.length >= 3) {
      return `${long[3]}-${months[complet]}-${long[1].padStart(2, '0')}`;
    }
  }
  return null;
}
