...
function recuperationsPour(user, ops) {
  ...
  // qu'un succès.
  //
  // ─── Une seule fin par connecteur : la DERNIÈRE (lot 71) ──────────────────
  //
  // Mesuré le 08/09 : Electro Dépôt échoue à 19:28, l'utilisateur se reconnecte,
  // la récupération réussit à 19:31 — et à 19:33 le bandeau annonce encore
  // « arrêtée sur un échec ». La fenêtre d'une heure vaut pour un échec NON
  // SUIVI : un succès postérieur du même connecteur remplace l'échec (et
  // s'efface ensuite selon sa propre fenêtre de deux minutes). Même idiome
  // `MAX(id)` que les blocs de l'accueil (recentErrors, home.js).
  const finies = db
    .get()
    .prepare(
      `SELECT r.connector_id, r.started_at, r.finished_at, r.success, r.invoice_count, r.message
         FROM run_logs r
        ...
        WHERE r.finished_at >= datetime('now', CASE r.success WHEN 0 THEN ? ELSE ? END)
        ORDER BY r.success ASC, r.finished_at DESC
        LIMIT ?`
    )
    .all(user.id, `-${FENETRE_ECHEC_MIN} minutes`, `-${FENETRE_SUCCES_MIN} minutes`, MAX_FINIES);

  for (const r of finies) {
    // La dernière EXÉCUTION du connecteur est celle qui tourne en ce moment :
    // sa fin précédente n'est plus « ce qui vient de se passer ».
    if (enCours.has(r.connector_id)) continue;
    ops.push({
