...
/** Attend qu'une condition devienne vraie — la boucle de surveillance est asynchrone. */
...
test('la fin de connexion détectée conduit la fenêtre sur l\'adresse de contrôle — sans aucun clic', async () => {
  // Lot 72 — mesuré le 08/09/2026 sur Boulanger : après le login, le site
  // dépose l'utilisateur sur les achats EN COURS (vides), et la vérification
  // automatique jugeait cette page-là — « non confirmée », fenêtre laissée sur
  // la mauvaise page. Dès que la connexion est jugée finie, la fenêtre est
  // conduite sur l'adresse déclarée, où les marqueurs concluent — clic ou pas.
  const page = pageFactice([
    { url: 'https://exemple.test/account/my-orders/in-progress' },
    {
      url: 'https://exemple.test/espace/mes-commandes',
  ...
  assert.ok(await attendre(() => captures.length > 0),
    'la session doit s\'enregistrer seule, une fois la fenêtre conduite');
  assert.ok(page.gotos.includes('https://exemple.test/espace/mes-commandes'),
    'la fenêtre est conduite sur l\'adresse de contrôle');
  assert.equal(contexte.manager.status(1, 'exemple')?.state, 'saved');
});

test('aucune conduite pendant qu\'un écran d\'authentification est affiché — le formulaire reste sous les doigts', async () => {
  // Même sur un clic « Enregistrer », une page d'authentification ne se
  // quitte pas d'autorité : naviguer emporterait le formulaire que
  // l'utilisateur est peut-être en train de remplir.
  const page = pageFactice([
    { url: 'https://exemple.test/connexion/etape-2' },
    {
      url: 'https://exemple.test/espace/mes-commandes',
      selecteurs: { '[data-testid="order"]': 3 },
    },
  ...
  assert.equal(page.gotos.length, 0,
    'la fenêtre ne bouge JAMAIS pendant qu\'un écran d\'authentification est affiché');
  assert.equal(captures.length, 0);
