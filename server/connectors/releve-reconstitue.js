...
/**
 * Les deux NATURES de document que ce module sait produire (lot 74).
 *
 * Un vocabulaire FERMÉ, et c'est le point : la phrase d'avertissement n'est
 * pas un paramètre libre. Un appelant choisit « relevé » (l'historique
 * d'opérations mis en forme — PayPal, Bitstamp) ou « reçu » (une commande
 * telle que l'espace client l'affiche — Atma) ; il ne peut ni écrire sa
 * propre phrase, ni l'affaiblir, ni l'enlever. Une nature inconnue est
 * refusée avant d'écrire quoi que ce soit.
 *
 * La nature par défaut est « relevé » : les documents PayPal et Bitstamp
 * sortent OCTET POUR OCTET comme avant ce lot.
 */
const NATURES = {
  releve: {
    titreBandeau: 'RELEVÉ RECONSTITUÉ — DOCUMENT PRODUIT PAR CRABE',
    phrase: (service) => `Relevé reconstitué par crabe à partir de l'historique d'opérations `
      + `— ceci n'est pas un document émis par ${service}.`,
    pied: 'Relevé reconstitué par crabe',
    titreDocument: (service) => `Relevé reconstitué — ${service}`,
    compteur: 'Opérations reprises',
  },
  recu: {
    titreBandeau: 'REÇU RECONSTITUÉ — DOCUMENT PRODUIT PAR CRABE',
    phrase: (service) => `Reçu reconstitué par crabe à partir de votre espace client `
      + `— ceci n'est pas une facture émise par ${service}.`,
    pied: 'Reçu reconstitué par crabe',
    titreDocument: (service) => `Reçu reconstitué — ${service}`,
    compteur: 'Lignes reprises',
  },
};

/** La nature demandée, ou une erreur : jamais un document sans sa mention. */
function natureDe(nature) {
  const retenue = NATURES[nature ?? 'releve'];
  if (!retenue) {
    throw new Error(
      `Nature de document reconstitué inconnue : « ${nature} » — natures possibles : `
        + Object.keys(NATURES).join(', ')
    );
  }
  return retenue;
}
 ...
 */
function bandeau(service, nature = 'releve') {
  return natureDe(nature).phrase(service);
}

/** Le titre du cadre, plus court et plus gros : ce qu'on voit avant de lire. */
const BANDEAU_TITRE = NATURES.releve.titreBandeau;
 ...
 */
function dessinerBandeau(service, nature = 'releve') {
  const y = PAGE_HAUTEUR - MARGE - BANDEAU_HAUTEUR;
  const phrase = bandeau(service, nature);
  ...
  return [
    ...
    texte('FHB', 11, MARGE + BANDEAU_RETRAIT, y + BANDEAU_HAUTEUR - 17, natureDe(nature).titreBandeau),
    texte('FCB', corps, MARGE + BANDEAU_RETRAIT, y + BANDEAU_HAUTEUR - 33, phrase),
  ].join('\n');
}

/** Le pied de page : ce que porte une page détachée du reste. */
function dessinerPied(service, compte, numero, total, nature = 'releve') {
  const gauche = `${natureDe(nature).pied} — ${service}${compte ? ` — compte ${compte}` : ''}`;
 ...
 * @param {'releve'|'recu'} [options.nature] la nature du document (lot 74) —
 *   choisie dans le vocabulaire fermé `NATURES`, jamais une phrase libre
...
function construire({
  ...
  const identite = [
    ...
    // Un reçu porte la date de SA commande (dans ses mentions), pas une
    // période : la ligne ne s'écrit alors que si l'appelant en donne une.
    // Pour un relevé, elle s'écrit toujours — documents inchangés.
    ...(nature === 'recu' && !periode.du && !periode.au
      ? []
    ...
    `${formes.compteur} : ${operations.length}`,
    ...mentions,
  ...
  const flux = pages.map((page, index) => {
    const morceaux = [dessinerBandeau(service, nature)];
    ...
    morceaux.push(dessinerPied(service, compte, index + 1, pages.length, nature));
    return morceaux.join('\n');
  });

  return assembler(flux, formes.titreDocument(service));
}
...
/**
 * Le nom de fichier d'un REÇU reconstitué (lot 74) : même logique, mais la
 * référence est celle de la commande, pas une période — un reçu par commande.
 * Le mot « recu-reconstitue » y figure pour la même raison que ci-dessus.
 */
function nomFichierRecu({ service, reference }) {
  const propre = (valeur) => String(valeur).toLowerCase().normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
  if (!reference) throw new Error('Un reçu reconstitué doit porter la référence de sa commande.');
  return `${propre(service)}_recu-reconstitue_${propre(reference)}.pdf`;
}
