...
test('un reçu français s\'imprime : numéro lu dans la SOURCE, A4, fonds compris', async () => {
  ...
  assert.equal(resultat.langue, 'française', 'la version française, cas nominal');
...
test('pas de version française atteignable : le reçu est imprimé dans la langue servie (lot 77)', async () => {
  const journal = [];
  const scenario = {
    lectures: [{ numero: 'GGL_I_0123456789', aTotalTTC: false, urlFrancais: null }],
  };
  const resultat = await deezer.imprimerRecu(
    fakeContexteImpression(scenario), LIEN_RECU, (m) => journal.push(m)
  );
  // Le français absent ne fait PLUS échouer le reçu : on l'archive dans la
  // langue servie, et le journal dit laquelle a été prise (choix du compte observé).
  assert.equal(resultat.ok, true, 'un reçu réel n\'est jamais perdu faute de français');
  assert.equal(resultat.numero, 'GGL_I_0123456789');
  assert.equal(resultat.langue, 'servie par le site');
  assert.equal(resultat.buffer.subarray(0, 5).toString(), '%PDF-');
  const texte = journal.join('\n');
  assert.match(texte, /langue servie par le site/);
  assert.match(texte, /n'a pas proposé « Français »/);
});
