...
function driverFor(destId) {
  ...
  DRIVERS[destId] = creerDestination({
    ...
    // ─── Une configuration inutilisable se refuse AVANT le service (lot 75) ──
    //
    // Une adresse WebDAV vide ou sans schéma est un échec certain : la jouer
    // ne peut produire qu'un « unsupported protocol scheme » en anglais —
    // mesuré à chaque « Tester » et chaque copie de la carte kDrive sans
    // numéro, du 07 au 09/09/2026. Le pilote porte donc le motif (même phrase
    // que le refus d'enregistrement) et s'arrête sans rien tenter.
    refusConfig: (normalise) =>
      (normalise.type === 'webdav' ? motifAdresseWebdav(ligne.provider, normalise) : null),
  });
...
/**
 * L'adresse WebDAV qu'une configuration fusionnée produirait vraiment.
 *
 * Trois provenances, celles que le pilote utilisera : l'adresse composée par
 * le preset (`versChamps` — kDrive fabrique la sienne depuis le numéro), le
 * champ `url` saisi (type webdav générique), ou une ligne `url = …` d'un bloc
 * rclone collé. Exportée pour être prouvée par les tests.
 */
function adresseWebdavEffective(provider, merged) {
  const preset = presets.of(provider);
  const composees = preset?.versChamps ? preset.versChamps(merged.valeurs || {}) : null;
  return String(
    (composees?.url ?? merged.valeurs?.url ?? '')
      || (/^\s*url\s*=\s*(\S+)\s*$/m.exec(String(merged.rcloneConfig || ''))?.[1] || '')
  ).trim();
}

/**
 * Ce qui rend une configuration WebDAV INUTILISABLE — le message qui dit le
 * champ à remplir, ou `null` si l'adresse tient debout.
 *
 * Le même motif sert TROIS endroits (lot 75) : le refus à l'enregistrement
 * (`verifierAdresseWebdav`), l'avertissement de la carte, et le refus avant
 * service du pilote — trois visages du même fait, une seule phrase. Mesuré le
 * 09/09/2026 : la carte kDrive enregistrée sans numéro AVANT la garde du
 * lot 74 continuait d'échouer à chaque « Tester » et à chaque copie, en
 * anglais (« unsupported protocol scheme »), sans que rien à l'écran ne dise
 * quoi remplir.
 */
function motifAdresseWebdav(provider, merged) {
  const adresse = adresseWebdavEffective(provider, merged);
  if (!adresse) {
    return String(provider) === 'kdrive'
      ? 'Le « Numéro de votre kDrive » n\'est pas renseigné : sans lui, crabe ne peut pas '
        + 'composer l\'adresse de connexion, et chaque copie échouerait. Recopiez ce numéro '
        + 'sur la carte kDrive — l\'aide du champ dit où le lire — puis enregistrez.'
      : 'L\'adresse du serveur WebDAV n\'est pas renseignée : sans elle, chaque copie '
        + 'échouerait. Renseignez le champ « URL of http host to connect to » — une adresse '
        + 'complète, qui commence par https:// — puis enregistrez.';
  }
  if (!/^https?:\/\//i.test(adresse)) {
    // Des identifiants glissés dans l'adresse (« compte:motdepasse@hôte ») ne
    // repartent pas dans un message qui finit à l'écran et au journal (lot 75).
    const citable = erreurs.masquerIdentifiantsUrl(adresse)
      .replace(/^[^\s/@]+(?::[^\s/@]*)?@/, '[masqué]@');
    return `L'adresse du serveur WebDAV (« ${citable.slice(0, 80)} ») est incomplète : elle doit `
      + 'commencer par https:// (ou http://). Recopiez l\'adresse complète depuis votre '
      + 'fournisseur, puis enregistrez.';
  }
  return null;
}

/**
 * Refuse (statut 400) une configuration WebDAV dont l'adresse est vide ou
 * sans schéma — voir l'appel dans `saveConfig` pour l'incident qui a fixé la
 * règle. La phrase dit le CHAMP à remplir, selon la carte d'où l'on vient.
 */
function verifierAdresseWebdav(provider, merged) {
  const motif = motifAdresseWebdav(provider, merged);
  if (!motif) return;
  const err = new Error(motif);
  err.statusCode = 400;
  throw err;
}
...
function publicConfig(destId) {
  ...
  return {
    ...
    // La ligne toujours visible du fournisseur (lot 76) : ce qu'il faut savoir
    // AVANT de remplir la carte — les offres kDrive sans WebDAV, aujourd'hui.
    // Du texte et un lien de documentation, jamais une valeur de l'utilisateur.
    note: preset.note,
...
async function publicConfigComplet(destId) {
  ...
  return {
    ...
    // La configuration INUTILISABLE passe en tête (lot 75) : la carte kDrive
    // sans numéro, enregistrée avant la garde du lot 74, échouait à chaque
    // geste sans qu'aucune ligne à l'écran ne dise le champ à remplir.
    avertissements: [
      ...(normalise.refusConfiguration ? [normalise.refusConfiguration] : []),
      ...presets.avertissements(normalise.type || base.type, {
        ...normalise,
...
function saveConfig(destId, values, champsConnus = []) {
  ...
  // ─── L'adresse WebDAV absente se refuse ICI, pas quatre copies plus tard ──
  //
  // Mesuré en production le 07/09/2026 (lot 74) : une carte kDrive enregistrée
  // avec l'adresse e-mail et le mot de passe, mais SANS le numéro du kDrive.
  // L'enregistrement est passé, et chaque copie a échoué sur « Propfind
  // "/crabe": unsupported protocol scheme "" » — une adresse vide, dite en
  // anglais, des jours après le geste qui l'a produite. Une destination
  // ACTIVE de type webdav sans adresse est un échec certain : on le dit au
  // moment d'enregistrer, avec le champ à remplir. Une destination désactivée
  // s'enregistre librement — refuser bloquerait jusqu'au geste qui la coupe.
  if (type === 'webdav' && enabledVoulu) {
    verifierAdresseWebdav(row(destId).provider, merged);
  }
...
function logResult(destId, userId, ok, message) {
  db.get()
    .prepare(
      'INSERT INTO destination_logs (dest_id, dest_name, user_id, success, message) '
        + 'VALUES (?, ?, ?, ?, ?)'
    )
    // Le nom s'écrit AVEC la ligne (lot 75) : un journal doit rester lisible
    // quand son sujet disparaît — résolu à l'affichage, le nom d'un cloud dont
    // la coquille a été nettoyée redevenait un identifiant technique.
    .run(destId, catalogue.style(destId).name, userId ?? null, ok ? 1 : 0, message || null);
}
...
/**
 * Le motif qui rend la configuration d'une destination INUTILISABLE, ou
 * `null` (lot 75).
 *
 * C'est la question que « Synchroniser » et la carte posent AVANT de tenter
 * quoi que ce soit : une adresse WebDAV absente ou sans schéma est un échec
 * certain, et le dire une fois avec le champ à remplir vaut mieux que
 * d'échouer document par document. Le motif vient du pilote
 * (`refusConfiguration`, calculé par `normalizeConf`) — le même que le refus
 * d'enregistrement et le refus avant service.
 */
function motifConfigInutilisable(destId) {
  if (destId === 'local') return null;
  const driver = driverFor(destId);
  if (!driver || typeof driver.normalizeConf !== 'function') return null;
  if (configIllisible(destId)) return null; // dit ailleurs, autrement (lot 45)
  const normalise = driver.normalizeConf(readConfig(destId));
  return normalise?.refusConfiguration || null;
}
...
module.exports = {
  ...
  localActif,
  saveConfig,
  adresseWebdavEffective,
  motifAdresseWebdav,
  motifConfigInutilisable,
  verifierAdresseWebdav,
  marquerSessionMorte,
