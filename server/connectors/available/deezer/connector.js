...
/**
 * Ouvre la page du reçu, préfère la version FRANÇAISE, et l'imprime en PDF —
 * exactement ce que ferait le bouton « Imprimer » du site, aucun contenu
 * fabriqué.
 *
 * Les preuves, dans l'ordre où elles tombent :
 *   1. la SOURCE porte le numéro de reçu AVANT le rendu — c'est l'ancre, et le
 *      PDF, en glyphes, ne se relira pas ;
 *   2. les octets rendus commencent par `%PDF-` ;
 *   3. le poids dépasse `SEUIL_PDF_OCTETS` — un rendu blanc est un ÉCHEC.
 *
 * ─── Lot 77 : le français est préféré, jamais exigé ──────────────────────────
 *
 * Le lot 41 REFUSAIT le reçu quand le sélecteur de langue n'offrait pas
 * « Français » (pas de « Total TTC » lu). Mesuré le 09/09/2026 : sur ce compte,
 * la bascule française réussit toujours — mais un reçu perdu parce que le site
 * ne propose plus le français serait pire que le même reçu archivé dans la
 * langue servie. La règle, désormais (choix du compte observé) : français si la page le
 * propose, SINON la langue servie, et le journal DIT laquelle a été prise. La
 * langue n'est plus une preuve — c'est le numéro de reçu qui ancre et prouve.
 *
 * @returns {Promise<{ok: true, numero: string, buffer: Buffer, langue: string}
...
async function imprimerRecu(context, lien, log = () => {}) {
  ...
  try {
    ...
    // Lot 77 — la langue prise, dite au journal. Le français reste préféré, mais
    // son absence n'est plus un échec : une bascule qui n'aboutit pas ne doit
    // pas faire perdre le reçu (choix du compte observé).
    const langue = recu.aTotalTTC ? 'française' : 'servie par le site';
    if (!recu.aTotalTTC) {
      log(`${ID} : reçu ${recu.numero} imprimé dans la langue servie par le site — le `
        + 'sélecteur de langue n\'a pas proposé « Français ». Rien n\'est perdu : le reçu est '
        + 'archivé tel que le site le montre.');
    }
    ...
    return { ok: true, numero: recu.numero, buffer, langue };
  } finally {
...
async function imprimerLesRecus(releve, context, { connus = new Set(), plan = null, log = () => {} } = {}) {
  ...
  for (const p of releve.paiements) {
    ...
    log(`${ID} : reçu ${resultat.numero} (paiement du ${p.dateBrute || '?'}) imprimé en PDF, `
      + `version ${resultat.langue} (${resultat.buffer.length} octets)`);
  }
