...
/**
 * Toutes les notifications d'un compte — lues comprises — pour l'écran
 * « Vos notifications » (lot 76), la plus récente en tête.
 *
 * Mesuré avant ce lot : quinze notifications écrites depuis le lot 66, TOUTES
 * non lues — parce qu'aucun écran ne les montrait. Les e-mails partaient, la
 * trace en base existait, et rien dans l'application ne la rattrapait.
 *
 * La borne est plus large que celle de `nonLues` : ici c'est un historique
 * qu'on consulte, pas une pastille qu'on relève. Au-delà, « Suivi actions »
 * sur l'accueil reste le vrai journal.
 */
function lister(userId, limite = 100) {
  return db
    .get()
    .prepare(
...
/** Combien de notifications ce compte n'a pas vues — le compteur de la cloche. */
function compterNonLues(userId) {
  return db
    .get()
    .prepare('SELECT COUNT(*) AS n FROM notifications WHERE user_id = ? AND seen_at IS NULL')
    .get(userId).n;
}
...
/**
 * Supprime CERTAINES notifications d'un compte — la croix d'une ligne (lot 79).
 *
 * Irréversible et volontairement non symétrique de `marquerVues` : sans `ids`
 * lisibles, rien n'est supprimé. Le « tout » est un autre geste (`oublier`),
 * demandé en toutes lettres et confirmé côté écran.
 */
function supprimer(userId, ids) {
  if (!Array.isArray(ids) || !ids.length) return 0;
  const marques = ids.map(() => '?').join(',');
  return db
    .get()
    .prepare(`DELETE FROM notifications WHERE user_id = ? AND id IN (${marques})`)
    .run(userId, ...ids).changes;
}

/**
 * Vide les notifications d'un compte (suppression RGPD, désinstallation — et,
 * depuis le lot 79, le « Tout supprimer » de l'écran, confirmé côté écran).
 */
