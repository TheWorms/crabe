...
function installedConnectors(user) {
  // ─── Les connecteurs EN PAUSE ne sont pas sur l'accueil (lot 82) ─────────
  //
  // C'est ici que la mise en pause se voit : le service disparaît des
  // vignettes, du bloc « Synchronisation », des gestes à faire et du décompte
  // « N connecteur(s) actif(s) » — tout ce que cet accueil sert à surveiller.
  // Un service en pause n'a rien à y faire : il ne va rien récupérer, il ne
  // peut rien échouer, et le voir figé là inquiéterait pour rien.
  //
  // ⚠ Sa carte reste sur Profil → Connecteurs installés, seul endroit d'où on
  // le rallume : le faire disparaître des DEUX écrans enfermerait
  // l'utilisateur dehors, sans porte de retour.
  return tri.parNom(
    registry.listForUser(user).filter((c) => c.installed && !c.disabledAt)
  );
}
...
function recentErrors(user, limit = 5) {
  ...
  //
  // ⚠ Sauf ceux mis en pause (lot 82). Chaque ligne d'ici porte l'action qui
  // RÉSOUT son échec — « Se reconnecter », « Reconfigurer » : la proposer pour
  // un service qu'on vient délibérément de mettre de côté serait le rappeler
  // à l'ordre pour une panne qu'on ne lui demande plus de régler.
  const catalog = new Map(
    registry.listForUser(user).filter((c) => !c.disabledAt).map((c) => [c.id, c])
  );
...
function recentSuccesses(user, limit = 6) {
  // Mêmes exclus qu'en face (recentErrors) : un service en pause quitte
  // l'accueil en entier, pas seulement par sa moitié rouge.
  const catalog = new Map(
    registry.listForUser(user).filter((c) => !c.disabledAt).map((c) => [c.id, c])
  );
