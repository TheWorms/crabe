...
function nextRunText(s) {
  // ⚠ Deux « aucune » différentes, et les confondre ferait chercher un
  // réglage de planification là où il n'y a rien à corriger : le service a
  // simplement été mis de côté depuis son profil (lot 82).
  if (s.disabledAt) return 'aucune — service en pause';
            ...
            isLocal
              ...
              : `<div class="field">
                   ...
                   <div class="field-help">Le nom que vous lui donnez, pour le reconnaître dans vos
                     écrans. Vous pouvez en avoir plusieurs chez le même fournisseur — un espace
                     perso et un espace boulot, par exemple.</div>
                 </div>
                 ${destTypeRow(d)}
                 ${destNoteProviderHtml(d)}
...
/**
 * La ligne toujours visible d'un fournisseur (lot 76) — kDrive, aujourd'hui :
 * quelles offres n'ouvrent pas l'accès WebDAV. La LIGNE ne se replie jamais
 * (c'est toute sa raison d'être : le dire AVANT le 403) ; le détail et le lien
 * vers la documentation du fournisseur vivent derrière « Tout lire », comme
 * les aides de champ.
 */
function destNoteProviderHtml(d) {
  const note = d.note;
  if (!note?.ligne) return '';
  const detail = note.detail
    ? `<details>
         <summary>Tout lire</summary>
         <div class="field-help-suite">${esc(note.detail)}
           ${note.lien?.href
             ? ` <a href="${esc(note.lien.href)}" target="_blank" rel="noopener">${esc(note.lien.texte || note.lien.href)}</a>`
             : ''}
         </div>
       </details>`
    : '';
  return `<div class="field-help dest-note-provider">${esc(note.ligne)}${detail}</div>`;
}

/**
 * L'avertissement de saisie d'un champ (lot 76) : le message à montrer pour la
 * valeur tapée, ou '' si elle n'appelle rien. Pur — le DOM est l'affaire de
 * `majAvertissementSaisie` — pour être prouvable hors navigateur.
 *
 * Un AVERTISSEMENT, jamais un refus : la valeur reste enregistrable telle
 * quelle (l'offre d'un fournisseur peut changer, un compte peut être mixte).
 */
function messageAvertissementSaisie(c, valeur) {
  const regle = c?.avertissementSaisie;
  if (!regle?.motif) return '';
  try {
    return new RegExp(regle.motif, 'i').test(String(valeur || '').trim())
      ? String(regle.message || '')
      : '';
  } catch {
    return '';
  }
...
/** Montre ou retire l'avertissement de saisie d'un champ, à chaque frappe. */
function majAvertissementSaisie(destId, cle) {
  const champ = $(`dest-${destId}-champ-${cle}`);
  const zone = $(`dest-${destId}-avert-${cle}`);
  if (!champ || !zone) return;
  const d = admin.destinations?.find((x) => x.id === destId);
  const c = champsDeLaCarte(d || {})?.find((x) => x.key === cle);
  const message = messageAvertissementSaisie(c, champ.value);
  zone.hidden = !message;
  if (message) zone.textContent = `⚠ ${message}`;
}
...
function champHtml(d, c) {
  ...
  // déjà tout seul, et la figerait à la version d'aujourd'hui. `exemple`
  // (lot 75) montre la FORME attendue de la même façon — toujours une valeur
  // générique du preset, jamais celle de quelqu'un.
  const filigrane = secretPose
    ? 'Enregistré — laisser vide pour le conserver'
    : (c.defaut && !valeur
      ? `par défaut : ${c.defaut}`
      : (c.exemple && !valeur ? `par exemple : ${c.exemple}` : ''));
  const saisie = c.options?.length
    ...
    : `<input type="${c.type === 'password' ? 'password' : 'text'}" ${commun}
              ...
              placeholder="${esc(filigrane)}"${
                c.avertissementSaisie
                  ? ` oninput="majAvertissementSaisie('${esc(d.id)}', '${esc(c.key)}')"`
                  : ''
              }>`;
  // L'avertissement de saisie (lot 76) : sa zone existe dès le rendu — remplie
  // si la valeur ENREGISTRÉE appelle déjà l'avertissement, sinon en attente de
  // la frappe. Jamais bloquant : la case reste saisissable et enregistrable.
  const avertissementInitial = c.avertissementSaisie ? messageAvertissementSaisie(c, valeur) : '';
  const zoneAvertissement = c.avertissementSaisie
    ? `<div class="sec-note" id="dest-${esc(d.id)}-avert-${esc(c.key)}"
            style="margin:6px 0 0;color:var(--amber);"${avertissementInitial ? '' : ' hidden'}>${
              avertissementInitial ? esc(`⚠ ${avertissementInitial}`) : ''
            }</div>`
    : '';
...
const LOGS_TRIS = {
  ...
  'logs-storage': {
    defaut: { key: 'date', dir: 'desc' },
    acces: {
      // Le nom peut être perdu (destination supprimée, lot 75) : le tri
      // range alors la ligne sous son libellé de remplacement.
      dest: (l) => l.destName || 'Destination supprimée',
      date: (l) => fmt.parse(l.at)?.getTime() || null,
      user: (l) => l.username || '',
      status: (l) => !!l.success,
      message: (l) => l.message || '',
...
/**
 * La cellule « Destination » d'une ligne du journal de stockage (lot 75).
 *
 * Un journal doit rester lisible quand son sujet disparaît : une destination
 * supprimée garde son nom (écrit avec la ligne depuis la migration 53) avec la
 * mention qu'elle n'existe plus ; quand le nom est déjà perdu, la ligne le dit
 * en français plutôt que d'afficher un identifiant technique — deux lignes
 * MEGA du 26/08 sous « cloud-1f2e3d4c » ont fait croire à des tentatives en
 * cours. La mention « désactivée » distingue, dans la vue Échecs, ce qui est
 * encore actif de ce qui est révolu.
 */
function destLogCell(l) {
  const mention = (texte) => `<span class="dest-tag">${texte}</span>`;
  if (l.destEtat === 'supprimee') {
    return l.destName
      ? `${esc(l.destName)} ${mention('destination supprimée')}`
      : '<span style="color:var(--muted);">Destination supprimée</span>';
  }
  const nom = esc(l.destName || l.dest_id || '—');
  return l.destEtat === 'desactivee' ? `${nom} ${mention('désactivée')}` : nom;
}
        ...
        data.logs.length
          ? trierLogs('logs-storage', data.logs, (l) => fmt.parse(l.at)?.getTime() || 0)
              .map(
                (l) => `
        <tr>
          <td data-label="Destination" style="color:var(--text);">${destLogCell(l)}</td>
          <td data-label="Date" title="${esc(fmt.exact(l.at))}">${esc(fmt.dateTime(l.at))}</td>
          <td data-label="Compte">${esc(l.username || '—')}</td>
