...
/**
 * Cette URL est-elle SUR le domaine déclaré ? (lot 73)
 *
 * `memeSite` est bidirectionnel : il rapproche deux adresses sans savoir
 * laquelle est « le domaine ». Ici la comparaison a un sens : un connecteur
 * DÉCLARE le domaine de son espace client (`remoteLogin.domaineEspaceClient`),
 * et on vérifie qu'une adresse y atterrit — `shopify.com` accepte
 * `shopify.com` et `account.shopify.com`, jamais `shopify.com.exemple.net` ni
 * l'inverse (`exemple.net` ne « contient » pas un sous-domaine shopify.com).
 *
 * Sert à la preuve d'un espace client hébergé AILLEURS que la boutique : les
 * comptes clients Shopify vivent sur `shopify.com`, pas sur la boutique
 * (mesuré sur Atma le 08/09/2026). Un renvoi VERS ce domaine est la preuve
 * attendue — avec les gardes du lot 68 conservées par l'appelant (statut ≥ 400
 * jamais une preuve, renvoi vers une page de connexion = refus).
 *
 * @param {string} url l'adresse atteinte
 * @param {string} domaine le domaine déclaré, ex. « shopify.com »
 */
function surDomaine(url, domaine) {
  let hote;
  try {
    hote = new URL(String(url)).hostname.toLowerCase().replace(/^www\./, '');
  } catch {
    return false;
  }
  const cible = String(domaine || '').trim().toLowerCase().replace(/^\./, '').replace(/^www\./, '');
  if (!hote || !cible) return false;
  return hote === cible || hote.endsWith(`.${cible}`);
}
...
function ligneConfirmee(nom, preuve) {
  // ⚠ Tolérante à un objet incomplet, comme ses trois sœurs du module
  // (`decrireMarqueur`, `decrireManque`, `ligneNonConfirmee`) : c'est une
  // fonction de JOURNAL, appelée dans le chemin nominal juste après une
  // connexion confirmée (prestashop, remote-browser). Une ligne de journal qui
  // lève ferait échouer le geste qu'elle documente — la récupération tomberait
  // en disant que la connexion a échoué, alors qu'elle venait d'être prouvée.
  // Le repli sur `marqueurs` est la forme héritée que `verifier()` remonte
  // encore explicitement (« Compatibilité avec les appelants qui lisaient
  // `marqueurs` »).
  const marqueur = preuve.preuvesFortes?.[0] ?? preuve.marqueurs?.[0];
  const morceaux = [`URL finale ${preuve.url || '(inconnue)'}`, decrireMarqueur(marqueur)];
