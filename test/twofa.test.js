...
// ---------------------------------------------------------------------------
// Vuln 1 (lot 83) — un compte à 2FA active ne peut pas ré-enrôler pour
// contourner le second facteur.
// ---------------------------------------------------------------------------

test('Vuln 1 : un compte à 2FA active ne peut pas se ré-enrôler pour sauter le second facteur', async (t) => {
  settings.setTwoFactorMode('allowed');
  t.after(() => settings.setTwoFactorMode('allowed'));

  const victime = await helpers.createUser({ username: 'victime2fa', plainPassword: 'MotDePasse1' });
  const secretLegitime = totp.generateSecret();
  helpers.db
    .get()
    .prepare('UPDATE users SET totp_enabled = 1, totp_secret = ? WHERE id = ?')
    .run(helpers.crypto.encrypt(secretLegitime), victime.id);

  const client = await helpers.startServer();
  t.after(() => client.close());

  // 1. Le seul mot de passe ouvre l'étape 2FA (session « pending »).
  const step = await client.post('/api/auth/login', {
    username: 'victime2fa',
    password: 'MotDePasse1',
  });
  assert.equal(step.body.step, '2fa');

  // 2. Demander un NOUVEAU secret d'enrôlement : refusé (400), aucun secret rendu.
  const setup = await client.post('/api/auth/2fa/setup');
  assert.equal(setup.status, 400);
  assert.match(setup.body.error, /déjà active/);
  assert.equal(setup.body.secret, undefined, 'aucun secret ne doit fuiter');

  // 3. Confirmer malgré tout : aucun enrôlement en cours → jamais un 200.
  const confirm = await client.post('/api/auth/2fa/confirm', { code: '000000' });
  assert.notEqual(confirm.status, 200);

  // 4. La session n'est JAMAIS authentifiée (c'est la preuve de la non-morsure).
  assert.equal((await client.get('/api/auth/me')).status, 401);

  // 5. Le secret légitime de la victime est intact — jamais écrasé.
  const row = userRow('victime2fa');
  assert.equal(row.totp_enabled, 1);
  assert.equal(
    helpers.crypto.decrypt(row.totp_secret),
    secretLegitime,
    'le secret légitime doit rester inchangé'
  );
});
