...
test('chaque ébauche dépose la preuve de liste et s\'arrête en le disant — jamais un « aucune nouvelle facture » muet', async () => {
  for (const { id, redirigeMesure } of SERVICES) {
    ...
    // Bricomarché non plus : le lot 78 a écrit le parcours de « Mon
    // historique d'achats » (facture téléchargée, ou reçu reconstitué quand
    // la commande n'en a pas) — ses tests vivent dans
    // lot78-ameli-bricomarche-caf.test.js.
    if (id === 'bricomarche') continue;
...
test('test() rend un verdict honnête : connexion valide, et ce qui viendra ensuite', async () => {
  // Bricomarché n'est plus une ébauche depuis le lot 78 : son test() dit que
  // l'historique d'achats sera lu à la prochaine récupération — plus jamais
  // « récupération pas écrite », qui serait devenu faux.
  ...
  assert.match(resultat.message, /historique d'achats sera lu/);
});
