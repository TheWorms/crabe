...
function erreurRenvoiVersAuthentification(nomService, precision = '') {
  // « plusieurs JOURS », pas « plusieurs semaines » (lot 76) : mesuré sur
  // l'historique Darty du 23/08 au 08/09/2026 — toutes les lectures réussies
  // sont survenues moins de deux heures après une connexion fraîche, et une
  // session de trois jours était déjà renvoyée (26/08). Bricomarché (lot 53)
  // se comptait même en minutes. Annoncer des semaines faisait attendre une
  // expiration déjà consommée.
  const err = new Error(
    ...
    + 'Si cette connexion date de plusieurs jours, elle a probablement expiré : rouvrez-la '
  ...
  err.sessionExpired = true;
  err.precision = precision;
  // Lot 71 : ce cas ne se reformule JAMAIS en « votre connexion a expiré »
  // selon l'historique — la session est peut-être valide et c'est le site qui
  // refuse le passage automatique. Le message ci-dessus dit déjà les deux
  // issues ; le marqueur le tient à l'écart de messageDeRefusExpire.
