...
// ---------------------------------------------------------------------------
// Vuln 2 (lot 83) — la route finalize exige une demande révoquée et protège
// les comptes administrateur.
// ---------------------------------------------------------------------------

test('Vuln 2 : la finalisation exige une demande de suppression révoquée', async (t) => {
  const admin = await helpers.createUser({ username: 'gardienne', role: 'admin' });
  helpers.db
    .get()
    .prepare('UPDATE users SET role_id = ? WHERE id = ?')
    .run(permissions.roleBySlug('admin').id, admin.id);
  const cible = await helpers.createUser({ username: 'ursule' });

  const client = await helpers.startServer();
  t.after(() => client.close());
  await helpers.login(client, 'gardienne', 'MotDePasse1');

  // a. Aucune demande → 409, et le compte survit (c'était LA faille : la route
  //    supprimait n'importe quel compte sans demande ni délai de grâce).
  const sansDemande = await client.post(`/api/users/deletion/requests/${cible.id}/finalize`);
  assert.equal(sansDemande.status, 409);
  assert.ok(
    helpers.db.get().prepare('SELECT id FROM users WHERE id = ?').get(cible.id),
    'le compte doit survivre à une finalisation sans demande'
  );

  // b. Demande NON révoquée → 409, le compte survit toujours.
  deletion.requestDeletion(cible.id, false);
  const nonRevoquee = await client.post(`/api/users/deletion/requests/${cible.id}/finalize`);
  assert.equal(nonRevoquee.status, 409);
  assert.ok(helpers.db.get().prepare('SELECT id FROM users WHERE id = ?').get(cible.id));

  // c. Après révocation → le workflow légitime aboutit (200, compte supprimé).
  deletion.revokeAccess(cible.id);
  const ok = await client.post(`/api/users/deletion/requests/${cible.id}/finalize`);
...
test('Vuln 2 : un gestionnaire délégué ne peut pas finaliser la suppression d\'un administrateur', async (t) => {
  const role = permissions.createRole('Assistance comptes', ['users.manage']);
  const helpdesk = await helpers.createUser({ username: 'assistance', plainPassword: 'MotDePasse1' });
  permissions.assignRole(helpdesk.id, role.id);
  const admin = helpers.db.get().prepare("SELECT * FROM users WHERE username = 'gardienne'").get();

  const client = await helpers.startServer();
  t.after(() => client.close());
  await helpers.login(client, 'assistance', 'MotDePasse1');

  const res = await client.post(`/api/users/deletion/requests/${admin.id}/finalize`);
  assert.equal(res.status, 403);
  assert.ok(
    helpers.db.get().prepare('SELECT id FROM users WHERE id = ?').get(admin.id),
    'l\'administrateur doit survivre'
  );
});
