...
test('Fiche non connectée : une phrase, un bouton, et rien d\'autre de visible', async () => {
  ...
  assertScreen(actions, {
    ...
    // Le bouton ENREGISTRE puis ouvre la fenêtre (un connecteur à session peut
    // porter des champs classiques — L'Atelier du Portable déclare `email`) :
    // `enregistrerPuisOuvrirConnexion` appelle `openRemoteLogin` derrière.
    contains: [
      'Se connecter à Free Mobile',
      "enregistrerPuisOuvrirConnexion('free-mobile')",
      'btn-remote',
    ],
  });
...
// ---------------------------------------------------------------------------
// Lot 72 — « Voir les documents » d'une fiche ouvre la liste FILTRÉE
// ---------------------------------------------------------------------------

test('la fiche d\'un connecteur ouvre « Mes documents » filtré sur lui — pas la liste globale', async () => {
  // Régression rapportée le 08/09/2026 : le comportement d'origine
  // (openPapiersFor) filtrait sur le connecteur courant ; le retrait de
  // « Mes Papiers » puis le retour de l'écran « Mes documents » avaient
  // rebranché le bouton de la fiche sur la liste globale, sans filtre.
  const app = makeSandbox(FIXTURES);
  // Une visite précédente de l'écran a laissé une recherche et une période :
  // elles cacheraient les documents qu'on vient précisément voir.
  app.run('docs.query.q = "reliquat"; docs.query.period = "2020"');
  app.run('state.currentConnectorId = "free-mobile"');
  app.run('openDocsForConnector()');
  await new Promise((resolve) => setImmediate(resolve));

  assert.equal(app.run('docs.query.connector'), 'free-mobile',
    'le filtre de l\'écran est posé sur le connecteur de la fiche');
  assert.equal(app.run('docs.query.q'), '', 'la recherche d\'une visite précédente est effacée');
  assert.equal(app.run('docs.query.period'), '', 'la période d\'une visite précédente est effacée');
  const requete = app.calls.api.find((u) => String(u).startsWith('/documents'));
  assert.ok(requete && requete.includes('connector=free-mobile'),
    `la liste est demandée FILTRÉE au serveur — reçu : ${requete}`);

  // Le bouton de la fiche (app.html) est bien branché sur cette fonction :
  // sans ce contrôle, la fonction pourrait exister et le bouton pointer
  // ailleurs — la régression exacte que ce test interdit.
  const gabarit = fs.readFileSync(path.join(WEB, 'app.html'), 'utf8');
  assert.match(gabarit, /qv-open-btn" onclick="openDocsForConnector\(\)"/,
    'le bouton « Voir les documents » de la fiche appelle openDocsForConnector');
});

// ---------------------------------------------------------------------------
// Lot 72 — l'accueil montre sa structure pendant le chargement
// ---------------------------------------------------------------------------

test('l\'accueil n\'arrive plus vide : squelette pendant l\'attente, contenu à l\'arrivée, jamais de gris au retour', async () => {
  const app = makeSandbox(FIXTURES);
  // Une réponse /home qui n'arrive pas tout de suite : ce que l'écran montre
  // PENDANT l'attente est exactement ce que ce test juge.
  app.run(`
    __enAttente = [];
    api = (path) => new Promise((resolve) => __enAttente.push(() => resolve(__fixture(path))));
  `);

  const premierRendu = app.run('renderHome()');
  const pendant = app.html('home-widgets');
  assert.ok(pendant.includes('skl-line'), 'des blocs gris pendant le chargement, pas une page vide');
  assert.ok(pendant.includes('widget-head'),
    'le squelette a la structure des vrais blocs — le contenu prendra leur place sans saut');
  assert.ok(pendant.includes('role="status"'), 'le lecteur d\'écran est prévenu que ça charge');
  assert.equal(app.run('__enAttente.length'), 1,
    'le squelette ne coûte AUCUNE requête : seule /home est partie');

  // La réponse arrive : le squelette laisse la place au contenu réel.
  app.run('__enAttente[0]()');
  await premierRendu;
  const rempli = app.html('home-widgets');
  assert.equal(rempli.includes('skl-line'), false, 'plus aucun bloc gris une fois les données là');
  assert.ok(rempli.includes('data-widget='), 'les vrais blocs sont rendus');

  // Retour sur l'accueil : les blocs de la visite précédente restent visibles
  // pendant le rafraîchissement — les remplacer par du gris serait un recul.
  app.run('renderHome()');
  const retour = app.html('home-widgets');
  assert.equal(retour.includes('skl-line'), false, 'jamais de squelette par-dessus un accueil déjà rendu');
  assert.ok(retour.includes('data-widget='), 'le contenu précédent reste à l\'écran');
});

// ---------------------------------------------------------------------------
// Lot 74 — l'accueil dit qu'il charge
// ---------------------------------------------------------------------------

test('le chargement se dit : texte « Chargement en cours… » et indicateur animé en tête, disparus au premier rendu', async () => {
  const app = makeSandbox(FIXTURES);
  app.run(`
    __enAttente = [];
    api = (path) => new Promise((resolve) => __enAttente.push(() => resolve(__fixture(path))));
  ...
  const premierRendu = app.run('renderHome()');
  const pendant = app.html('home-widgets');
  assert.ok(pendant.includes('Chargement en cours…'),
    'le texte dit que la page travaille — la structure grise seule pouvait passer pour une page cassée');
  assert.ok(pendant.includes('class="spinner"'), 'un indicateur animé accompagne le texte');
  assert.match(pendant, /skl-chargement" role="status"/,
    'role=status conservé, porté par l\'indicateur désormais visible');
  assert.equal(pendant.split('skl-chargement').length - 1, 1,
    'UN SEUL indicateur, en tête — pas un par bloc');
  assert.ok(pendant.indexOf('skl-chargement') < pendant.indexOf('widget-head'),
    'l\'indicateur précède le premier bloc gris');
  assert.equal(app.run('__enAttente.length'), 1,
    'l\'indicateur ne coûte AUCUNE requête : seule /home est partie');

  app.run('__enAttente[0]()');
  await premierRendu;
  const rempli = app.html('home-widgets');
  assert.equal(rempli.includes('skl-chargement'), false,
    'l\'indicateur disparaît avec le premier rendu réel');

  // La feuille de style tient les deux promesses d'accessibilité : l'indicateur
  // est visible (pas le motif lecteur-d'écran-seul de l'ancienne note), et sous
  // « moins d'animations » la rotation se coupe — le texte reste.
  const css = fs.readFileSync(path.join(WEB, 'style.css'), 'utf8');
  assert.match(css, /\.skl-chargement\{grid-column:1 \/ -1/,
    'l\'indicateur occupe la tête de grille, visible — pas masqué comme la note du lot 72');
  assert.match(css, /prefers-reduced-motion:reduce\)\{\.skl-line,\.skl-chargement \.spinner\{animation:none;\}/,
    'sous prefers-reduced-motion, plus d\'animation : le texte suffit');
});
