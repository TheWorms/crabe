...
function listInstallations() {
  const rows = db
    ...
    .prepare(
      `SELECT ci.user_id, ci.connector_id, ci.status AS install_status,
              ...
              ci.last_run_at, ci.last_error, ci.disabled_at,
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      // Mis en pause par l'utilisateur (lot 82) : la planification reste
      // enregistrée telle quelle — c'est le planificateur qui n'arme pas sa
      // tâche, et qui la réarmera au rallumage sans rien avoir à reconstruire.
