...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Un compte qui a DÉJÀ la 2FA ne repasse pas par l'enrôlement : sans cette
    // garde, quiconque connaît le mot de passe pouvait générer un nouveau secret
    // et se connecter sans le second facteur légitime. Même garde que /2fa/skip.
    // Pour remplacer son secret, l'utilisateur la désactive d'abord (/2fa/disable).
    if (user.totp_enabled && user.totp_secret) {
      return res.status(400).json({
        error:
          'La double authentification est déjà active sur ce compte. Pour la remplacer, désactivez-la d\'abord depuis votre profil, puis réactivez-la.',
      });
    }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Écriture conditionnelle (défense en profondeur) : on n'active la 2FA que
    // si le compte ne l'avait pas déjà. Un secret déjà en place n'est jamais
    // écrasé — la garde de /2fa/setup en amont, et celle-ci en aval.
    const written = db
      .get()
      .prepare('UPDATE users SET totp_secret = ?, totp_enabled = 1 WHERE id = ? AND totp_enabled = 0')
      .run(crypto.encrypt(secret), userId);
    if (written.changes === 0) {
      delete req.session.enrollSecret;
      delete req.session.enrollUserId;
      return res.status(409).json({
        error: 'La double authentification est déjà active sur ce compte.',
      });
    }
