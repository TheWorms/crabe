...
/**
 * L'écran « Vos notifications » (lot 76) : tout l'historique de CE compte —
 * lues comprises — et le compte des non-lues pour la pastille de la cloche.
 * Tout vient de la base locale : aucune sonde réseau, jamais.
 */
router.get('/me/notifications/liste', requireAuth, (req, res) => {
  res.json({
    notifications: notifications.lister(req.user.id),
    nonLues: notifications.compterNonLues(req.user.id),
  });
});
...
/**
 * Supprime des notifications (lot 79) — la croix d'une ligne, ou le geste
 * « Tout supprimer » de l'écran.
 *
 * La suppression est irréversible, alors le TOUT est un choix explicite
 * (`tout: true`) : un corps vide ou des `ids` illisibles ne suppriment RIEN —
 * jamais la symétrie de `seen`, où le vide veut dire « tout ». La réponse
 * porte le compte des non-lues restantes : la pastille de la cloche se met à
 * jour sans second appel.
 */
router.post('/me/notifications/supprimer', requireAuth, (req, res) => {
  const tout = req.body?.tout === true;
  const ids = Array.isArray(req.body?.ids)
    ? req.body.ids.map(Number).filter(Number.isInteger)
    : [];
  const supprimees = tout
    ? notifications.oublier(req.user.id)
    : notifications.supprimer(req.user.id, ids);
  res.json({ ok: true, supprimees, nonLues: notifications.compterNonLues(req.user.id) });
});
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Rôle visé : explicite (roleId) ou rôle intégré selon la nature demandée.
    // Un gestionnaire délégué ne peut pas créer un compte plus puissant que lui
    // (un administrateur porte dérobée, notamment).
    const wantedRoleId = req.body?.roleId
      ? Number(req.body.roleId)
      : permissions.defaultRoleIdFor(role === 'admin' ? 'admin' : 'user');
    const wantedRole = wantedRoleId ? permissions.roleById(wantedRoleId) : null;
    if (req.body?.roleId !== undefined && !wantedRole) {
      return res.status(404).json({ error: 'Rôle introuvable.' });
    }
    if (wantedRole && !permissions.canGrantRole(req.user, wantedRole)) {
      return res.status(403).json({
        error: 'Vous ne pouvez pas créer un compte doté de plus de droits que les vôtres.',
      });
    }
    ...
    const id = db
      ...
      .run(
        ...
        wantedRole && wantedRole.slug === permissions.ADMIN_SLUG ? 'admin' : 'user'
      ).lastInsertRowid;

    if (wantedRole) permissions.assignRole(id, wantedRole.id);
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Un compte non administrateur ne peut pas gérer un administrateur (reset de
    // mot de passe, changement de statut…) : sinon `users.manage` permettait de
    // prendre le contrôle du vrai administrateur.
    if (!permissions.canManageAccount(req.user, user)) {
      return res
        .status(403)
        .json({ error: 'Ce compte est administrateur : seul un administrateur peut le gérer.' });
    }

    // On ne modifie pas son propre rôle ni son propre statut par cette route :
    // c'est le vecteur d'auto-promotion. Un administrateur reste, lui, soumis au
    // garde-fou « dernier administrateur » plus bas.
    if (
      user.id === req.user.id &&
      req.user.role !== 'admin' &&
      (req.body?.roleId !== undefined || role !== undefined || status !== undefined)
    ) {
      return res.status(403).json({
        error: 'Vous ne pouvez pas modifier votre propre rôle ni votre propre statut.',
      });
    }
    ...
    if (req.body?.roleId !== undefined) {
      ...
      if (!permissions.canGrantRole(req.user, wanted)) {
        return res.status(403).json({
      ...
      const wanted = permissions.roleById(permissions.defaultRoleIdFor(role));
      if (!permissions.canGrantRole(req.user, wanted)) {
        return res.status(403).json({
          error: 'Vous ne pouvez pas attribuer un rôle doté de plus de droits que les vôtres.',
        });
...
router.post('/:id/2fa/reset', (req, res) => {
  ...
  if (!permissions.canManageAccount(req.user, user)) {
    return res.status(403).json({
      error: 'Ce compte est administrateur : seul un administrateur peut réinitialiser sa double authentification.',
    });
  }
...
router.post('/:id/revoke', (req, res) => {
  ...
  if (!permissions.canManageAccount(req.user, user)) {
    return res.status(403).json({
      error: 'Ce compte est administrateur : seul un administrateur peut révoquer son accès.',
    });
  }
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!permissions.canManageAccount(req.user, user)) {
      return res.status(403).json({
        error: 'Ce compte est administrateur : seul un administrateur peut le supprimer.',
      });
    }
    if (user.role === 'admin' && isLastAdmin(user.id)) {
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    // `:id` est un identifiant UTILISATEUR (comme /export et /revoke voisins, et
    // comme l'appelle web/admin.js). La faille corrigée ici n'était pas cette
    // interprétation mais l'ABSENCE des gardes ci-dessous : la route effaçait
    // n'importe quel compte, sans demande, sans révocation, dernier admin inclus.
    const user = row(req.params.id);
    if (!user) return res.status(404).json({ error: 'Utilisateur introuvable.' });
    if (!permissions.canManageAccount(req.user, user)) {
      return res.status(403).json({
    ...
    // La finalisation est le bout du parcours : une demande de suppression doit
    // exister ET avoir été révoquée (délai de grâce écoulé, ou révocation
    // explicite). Sans elle, on saute tout le workflow d'export et de préavis.
    const request = deletion.getRequestForUser(user.id);
    if (!request || !request.revoked) {
      return res.status(409).json({
        error:
          'Aucune suppression révoquée en attente pour ce compte : il faut d\'abord révoquer l\'accès.',
      });
    }
    ...
    applog.admin(
      req,
      `Compte « ${user.username} » supprimé définitivement (demande finalisée).`,
      'warn'
    );
    res.json({ ok: true, ...result });
  })
