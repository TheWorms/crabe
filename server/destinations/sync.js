...
function start({ destinationIds, userId = null, actor = null }) {
  ...
  // ─── Une configuration inutilisable ne se synchronise pas (lot 75) ─────────
  //
  // Mesuré du 07 au 09/09/2026 : la carte kDrive sans numéro (enregistrée
  // avant la garde du lot 74) aurait fait échouer la reprise document par
  // document, trois tentatives chacun, pour un défaut connu AVANT la première
  // copie. Le refus arrive donc ici, avec le champ à remplir. Quand la
  // synchronisation est globale, les destinations jouables partent quand
  // même : leur rattrapage n'a pas à attendre qu'une carte cassée soit
  // réparée — la carte cassée, elle, est dite dans le compte rendu.
  const motifs = new Map();
  const jouables = cibles.filter((id) => {
    const motif = destinations.motifConfigInutilisable(id);
    if (motif) motifs.set(id, motif);
    return !motif;
  });
  if (!jouables.length) {
    const err = new Error([...motifs.values()][0]);
    err.statusCode = 400;
    throw err;
  }

  const total = jouables.reduce((n, id) => n + pendingCount(id, userId), 0);

  etat = {
    ...repos(),
    running: true,
    destinationIds: jouables,
    scope: userId ? `user:${userId}` : 'all',
    startedAt: new Date().toISOString(),
    total,
    message: total ? 'Synchronisation en cours…' : 'Rien à synchroniser.',
  };
  for (const [destId, motif] of motifs) {
    noter(destId, null, `non synchronisée — ${motif}`);
  }

  // Détaché : la requête HTTP ne l'attend pas.
  (async () => {
    try {
      for (const destId of jouables) {
        await syncDestination(destId, { userId, actor });
      }
      etat.message = resume(etat);
      applog[etat.failed ? 'warn' : 'info'](
        'destinations',
        `Synchronisation forcée (${jouables.join(', ')}) : ${etat.message}`,
        actor || {}
      );
    } catch (err) {
